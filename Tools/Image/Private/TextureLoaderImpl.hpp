...
namespace shz
{
	...
	public:
		...
		TextureLoaderImpl(IReferenceCounters* pRefCounters, const TextureLoadInfo& TexLoadInfo, RefCntAutoPtr<Image>   pImage);

		IMPLEMENT_QUERY_INTERFACE_IN_PLACE(IID_TextureLoader, TBase)

			virtual void SHZ_CALL_TYPE CreateTexture(IRenderDevice* pDevice, ITexture** ppTexture) override final;

		virtual const TextureDesc& SHZ_CALL_TYPE GetTextureDesc() const override final
		{
			return m_TexDesc;
		}
		...
		virtual TextureData SHZ_CALL_TYPE GetTextureData() override final
		{
