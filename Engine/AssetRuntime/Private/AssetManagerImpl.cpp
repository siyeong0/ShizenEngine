...
namespace shz
{
	void AssetManagerImpl::RegisterAsset(const AssetID& id, AssetTypeID typeId, const std::string& sourcePath)
	{
	...
	void AssetManagerImpl::UnregisterAsset(const AssetID& id)
	{
		m_Registry.Unregister(id);
	}

	void AssetManagerImpl::RegisterLoader(AssetTypeID typeId, LoaderFn loader)
	{
	...
	void AssetManagerImpl::AddStrongRef(const AssetID& id, AssetTypeID typeId) noexcept
	{
	...
	void AssetManagerImpl::ReleaseStrongRef(const AssetID& id, AssetTypeID typeId) noexcept
	{
	...
	void AssetManagerImpl::RequestLoad(const AssetID& id, AssetTypeID typeId, uint32 flags)
	{
	...
	EAssetStatus AssetManagerImpl::GetStatusByID(const AssetID& id, AssetTypeID typeId) const noexcept
	{
	...
	AssetObject* AssetManagerImpl::TryGetByID(const AssetID& id, AssetTypeID typeId) noexcept
	{
	...
	const AssetObject* AssetManagerImpl::TryGetByIDConst(const AssetID& id, AssetTypeID typeId) const noexcept
	{
	...
	void AssetManagerImpl::WaitByID(const AssetID& id, AssetTypeID typeId) const
	{
	...
	bool AssetManagerImpl::Unload(const AssetID& id)
	{
	...
	void AssetManagerImpl::CollectGarbage()
	{
	...
	void AssetManagerImpl::Tick(float /*deltaSeconds*/)
	{
	...
	AssetRecord* AssetManagerImpl::getRecordOrNull_NoLock(const AssetID& id) noexcept
	{
		auto it = m_Records.find(id);
		return (it != m_Records.end()) ? it->second.get() : nullptr;
	}

	const AssetRecord* AssetManagerImpl::getRecordOrNull_NoLock(const AssetID& id) const noexcept
	{
		auto it = m_Records.find(id);
		return (it != m_Records.end()) ? it->second.get() : nullptr;
	}

	AssetRecord& AssetManagerImpl::getOrCreateRecord_NoLock(const AssetID& id, AssetTypeID typeId)
	{
	...
	void AssetManagerImpl::loadNow(AssetRecord& record)
	{
		AssetRegistry::AssetMeta meta = {};
		LoaderFn loader;
	...
	bool AssetManagerImpl::isPinned_NoLock(const AssetRecord& rec) const noexcept
	{
	...
	bool AssetManagerImpl::unloadRecord_NoLock(AssetRecord& rec)
	{
		std::unique_lock<std::mutex> lock(rec.Mutex);
	...
	void AssetManagerImpl::touchRecord_NoLock(const AssetRecord& rec) const noexcept
	{
		const uint64 frame = m_FrameIndex.load(std::memory_order_relaxed);
