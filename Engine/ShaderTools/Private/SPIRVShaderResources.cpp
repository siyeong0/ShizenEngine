...
#include "Engine/RHI/Public/ShaderBase.hpp"
...
namespace shz
{
	...
	Type GetResourceArraySize(
		const shizen_spirv_cross::Compiler& Compiler,
		const shizen_spirv_cross::Resource& Res)
	{
		const shizen_spirv_cross::SPIRType& type = Compiler.get_type(Res.type_id);
	...
	static RESOURCE_DIMENSION GetResourceDimension(
		const shizen_spirv_cross::Compiler& Compiler,
		const shizen_spirv_cross::Resource& Res)
	{
		const shizen_spirv_cross::SPIRType& type = Compiler.get_type(Res.type_id);
		if (type.basetype == shizen_spirv_cross::SPIRType::BaseType::Image ||
			type.basetype == shizen_spirv_cross::SPIRType::BaseType::SampledImage)
		{
	...
	static bool IsMultisample(const shizen_spirv_cross::Compiler& Compiler,
		const shizen_spirv_cross::Resource& Res)
	{
		const shizen_spirv_cross::SPIRType& type = Compiler.get_type(Res.type_id);
		if (type.basetype == shizen_spirv_cross::SPIRType::BaseType::Image ||
	...
	static uint32_t GetDecorationOffset(
		const shizen_spirv_cross::Compiler& Compiler,
		const shizen_spirv_cross::Resource& Res,
	...
	SPIRVShaderResourceAttribs::SPIRVShaderResourceAttribs(
		...
		, Type{ _Type }
		, ResourceDim{ shz::GetResourceDimension(Compiler, Res) }
		, IsMS{ shz::IsMultisample(Compiler, Res) ? uint8{1} : uint8{0} }
	...
	const std::string& GetUBOrSBName(shizen_spirv_cross::Compiler& Compiler,
		const shizen_spirv_cross::Resource& UB,
		const shizen_spirv_cross::ParsedIR::Source& IRSource)
	{
	...
	static SHADER_CODE_BASIC_TYPE SpirvBaseTypeToShaderCodeBasicType(shizen_spirv_cross::SPIRType::BaseType SpvBaseType)
	{
		switch (SpvBaseType)
		{
			
		case shizen_spirv_cross::SPIRType::Unknown:               return SHADER_CODE_BASIC_TYPE_UNKNOWN;
		case shizen_spirv_cross::SPIRType::Void:                  return SHADER_CODE_BASIC_TYPE_VOID;
		case shizen_spirv_cross::SPIRType::Boolean:               return SHADER_CODE_BASIC_TYPE_BOOL;
		case shizen_spirv_cross::SPIRType::SByte:                 return SHADER_CODE_BASIC_TYPE_INT8;
		case shizen_spirv_cross::SPIRType::UByte:                 return SHADER_CODE_BASIC_TYPE_UINT8;
		case shizen_spirv_cross::SPIRType::Short:                 return SHADER_CODE_BASIC_TYPE_INT16;
		case shizen_spirv_cross::SPIRType::UShort:                return SHADER_CODE_BASIC_TYPE_UINT16;
		case shizen_spirv_cross::SPIRType::Int:                   return SHADER_CODE_BASIC_TYPE_INT;
		case shizen_spirv_cross::SPIRType::UInt:                  return SHADER_CODE_BASIC_TYPE_UINT;
		case shizen_spirv_cross::SPIRType::Int64:                 return SHADER_CODE_BASIC_TYPE_INT64;
		case shizen_spirv_cross::SPIRType::UInt64:                return SHADER_CODE_BASIC_TYPE_UINT64;
		case shizen_spirv_cross::SPIRType::AtomicCounter:         return SHADER_CODE_BASIC_TYPE_UNKNOWN;
		case shizen_spirv_cross::SPIRType::Half:                  return SHADER_CODE_BASIC_TYPE_FLOAT16;
		case shizen_spirv_cross::SPIRType::Float:                 return SHADER_CODE_BASIC_TYPE_FLOAT;
		case shizen_spirv_cross::SPIRType::Double:                return SHADER_CODE_BASIC_TYPE_DOUBLE;
		case shizen_spirv_cross::SPIRType::Struct:                return SHADER_CODE_BASIC_TYPE_UNKNOWN;
		case shizen_spirv_cross::SPIRType::Image:                 return SHADER_CODE_BASIC_TYPE_UNKNOWN;
		case shizen_spirv_cross::SPIRType::SampledImage:          return SHADER_CODE_BASIC_TYPE_UNKNOWN;
		case shizen_spirv_cross::SPIRType::Sampler:               return SHADER_CODE_BASIC_TYPE_UNKNOWN;
		case shizen_spirv_cross::SPIRType::AccelerationStructure: return SHADER_CODE_BASIC_TYPE_UNKNOWN;
	...
	void LoadShaderCodeVariableDesc(
		const shizen_spirv_cross::Compiler& Compiler,
		const shizen_spirv_cross::TypeID& TypeID,
		const shizen_spirv_cross::Bitset& Decoration,
		bool                                  IsHLSLSource,
		ShaderCodeVariableDescX& TypeDesc)
	{
		const shizen_spirv_cross::SPIRType& SpvType = Compiler.get_type(TypeID);
		if (SpvType.basetype == shizen_spirv_cross::SPIRType::Struct)
		{
	...
	ShaderCodeBufferDescX LoadUBReflection(const shizen_spirv_cross::Compiler& Compiler, const shizen_spirv_cross::Resource& UB, bool IsHLSLSource)
	{
		const shizen_spirv_cross::SPIRType& SpvType = Compiler.get_type(UB.type_id);
		...
		m_ShaderType{ shaderDesc.ShaderType }
	{
		...
		shizen_spirv_cross::Parser parser{ std::move(spirv_binary) };
		parser.parse();
		const shizen_spirv_cross::ParsedIR::Source ParsedIRSource = parser.get_parsed_ir().source;

		m_IsHLSLSource = ParsedIRSource.hlsl;
		shizen_spirv_cross::Compiler Compiler{ std::move(parser.get_parsed_ir()) };

		spv::ExecutionModel ExecutionModel = ShaderTypeToSpvExecutionModel(shaderDesc.ShaderType);
		auto                EntryPoints = Compiler.get_entry_points_and_stages();
		for (const shizen_spirv_cross::EntryPoint& CurrEntryPoint : EntryPoints)
		{
		...
		shizen_spirv_cross::ShaderResources resources = Compiler.get_shader_resources();

		size_t ResourceNamesPoolSize = 0;
		for (const shizen_spirv_cross::Resource& ub : resources.uniform_buffers)
			ResourceNamesPoolSize += GetUBOrSBName(Compiler, ub, ParsedIRSource).length() + 1;
		for (const shizen_spirv_cross::Resource& sb : resources.storage_buffers)
		...
		{
			for (const shizen_spirv_cross::Resource& res : *pResType)
		...
		if (LoadShaderStageInputs)
		{
			...
			if (HlslFunctionality1)
			{
				for (const shizen_spirv_cross::Resource& Input : resources.stage_inputs)
				{
...

		{
			...
			for (const shizen_spirv_cross::Resource& UB : resources.uniform_buffers)
			{
				...
				const shizen_spirv_cross::SPIRType& Type = Compiler.get_type(UB.type_id);
...

		{
			...
			for (const shizen_spirv_cross::Resource& SB : resources.storage_buffers)
			{
				...
				shizen_spirv_cross::Bitset BufferFlags = Compiler.get_buffer_block_flags(SB.id);
				...
				const shizen_spirv_cross::SPIRType& Type = Compiler.get_type(SB.type_id);
...

		{
			...
			for (const shizen_spirv_cross::Resource& SmplImg : resources.sampled_images)
			{
				const shizen_spirv_cross::SPIRType& type = Compiler.get_type(SmplImg.type_id);
		...
		{
			uint32 CurrImg = 0;
			for (const shizen_spirv_cross::Resource& Img : resources.storage_images)
			{
				const shizen_spirv_cross::SPIRType& type = Compiler.get_type(Img.type_id);
		...
		{
			uint32 CurrAC = 0;
			for (const shizen_spirv_cross::Resource& AC : resources.atomic_counters)
			{
...

		{
			...
			for (const shizen_spirv_cross::Resource& SepSam : resources.separate_samplers)
			{
		...
		{
			uint32 CurrSepImg = 0;
			for (const shizen_spirv_cross::Resource& SepImg : resources.separate_images)
			{
				const shizen_spirv_cross::SPIRType& type = Compiler.get_type(SepImg.type_id);
...

		{
			...
			for (const shizen_spirv_cross::Resource& SubpassInput : resources.subpass_inputs)
			{
...

		{
			...
			for (const shizen_spirv_cross::Resource& AccelStruct : resources.acceleration_structures)
			{
	...
	void SPIRVShaderResources::Initialize(
		IMemoryAllocator& Allocator,
