...
#define FOURCC(a, b, c, d) (uint32{(d) << 24} | uint32{(c) << 16} | uint32{(b) << 8} | uint32{a})
		...
		{
		public:
			ShaderBytecodeRemapper(
				DataBlobImpl::DataBufferType& _Bytecode, size_t _ChunkOffset, TExtendedResourceMap& _ExtResMap, ResourceBindingPerType const& _BindingsPerType)
				: Bytecode{ _Bytecode }
