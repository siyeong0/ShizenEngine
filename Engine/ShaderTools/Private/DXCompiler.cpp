...
namespace shz
{
	...
	namespace
	{
		...
		public:
			...
			virtual void GetD3D12ShaderReflection(IDxcBlob* pShaderBytecode, ID3D12ShaderReflection** ppShaderReflection) override final;

			virtual bool RemapResourceBindings(
...
#undef CHECK_PATCHING_ERROR
	...
	// DXIL container format (minimal) parsing without dxc internal headers.
	// Reference: DXIL container format docs (FourCC 'DXIL', container 'DXBC')
	static inline uint32_t ReadU32LE(const uint8_t* p)
	{
		return (uint32_t)p[0] | ((uint32_t)p[1] << 8) | ((uint32_t)p[2] << 16) | ((uint32_t)p[3] << 24);
	}

	static inline bool HasRoom(const uint8_t* cur, const uint8_t* end, size_t bytes)
	{
		return cur + bytes <= end;
	}

	static inline uint32_t MakeFourCC(char a, char b, char c, char d)
	{
	...
	bool IsDXILBytecode(const void* pBytecode, size_t Size)
	{
		if (pBytecode == nullptr || Size < 4)
			return false;

		const uint8_t* begin = static_cast<const uint8_t*>(pBytecode);
		const uint8_t* end = begin + Size;

		// DXIL container starts with FourCC "DXBC"
		// (Yes, DXIL containers use DXBC container header, with parts including 'DXIL'.)
		if (!HasRoom(begin, end, 4))
			return false;

		const uint32_t ContainerFourCC = ReadU32LE(begin);
		if (ContainerFourCC != MakeFourCC('D', 'X', 'B', 'C'))
			return false;

		// Minimal container header layout (bytes):
		// 0x00: uint32 FourCC = 'DXBC'
		// 0x04: uint32 Hash[4]  (16 bytes)
		// 0x14: uint32 Version? (often 1)  (4 bytes)
		// 0x18: uint32 ContainerSize        (4 bytes)
		// 0x1C: uint32 PartCount            (4 bytes)
		// 0x20: uint32 PartOffset[PartCount] ...
		//
		// We only need PartCount and PartOffsets safely.

		const size_t HeaderBytes = 0x20;
		if (!HasRoom(begin, end, HeaderBytes))
			return false;

		const uint32_t PartCount = ReadU32LE(begin + 0x1C);

		// Sanity: avoid ridiculous counts causing overflow
		if (PartCount == 0 || PartCount > 4096)
			return false;

		const size_t OffsetsBytes = (size_t)PartCount * sizeof(uint32_t);
		if (!HasRoom(begin + 0x20, end, OffsetsBytes))
			return false;

		const uint32_t DXIL_FourCC = MakeFourCC('D', 'X', 'I', 'L');

		for (uint32_t i = 0; i < PartCount; ++i)
		{
			const uint32_t PartOffset = ReadU32LE(begin + 0x20 + (size_t)i * 4);

			// Each part begins with:
			// uint32 PartFourCC;
			// uint32 PartSize;
			if (PartOffset > Size || !HasRoom(begin + PartOffset, end, 8))
				return false;

			const uint32_t PartFourCC = ReadU32LE(begin + PartOffset + 0);
			const uint32_t PartSize = ReadU32LE(begin + PartOffset + 4);

			// Part payload begins at PartOffset+8, length PartSize.
			// Validate bounds (PartSize may be 0 for some parts but still valid)
			if (!HasRoom(begin + PartOffset + 8, end, (size_t)PartSize))
				return false;

			if (PartFourCC == DXIL_FourCC)
			{
				return true; // Found DXIL part
			}
		}
