...
namespace
{
    shz::RENDER_DEVICE_TYPE g_DeviceType = shz::RENDER_DEVICE_TYPE_UNDEFINED;
        ...
        LPARAM lParam)
    {
        ...
        case WM_COMMAND:
            ...
            {
            case ID_DIRECT3D11:
                g_DeviceType = shz::RENDER_DEVICE_TYPE_D3D11;
                EndDialog(hwndDlg, wParam);
                return TRUE;

            case ID_DIRECT3D12:
                g_DeviceType = shz::RENDER_DEVICE_TYPE_D3D12;
                EndDialog(hwndDlg, wParam);
                return TRUE;

            case ID_OPENGL:
                g_DeviceType = shz::RENDER_DEVICE_TYPE_GL;
                EndDialog(hwndDlg, wParam);
                return TRUE;

            case ID_VULKAN:
                g_DeviceType = shz::RENDER_DEVICE_TYPE_VULKAN;
                EndDialog(hwndDlg, wParam);
                return TRUE;
            }
            break;
...
namespace shz
{
    ...
    public:
        virtual LRESULT HandleWin32Message(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) override final
        {
            ...
            if (m_pImGui)
            {
                auto Handled = static_cast<ImGuiImplWin64*>(m_pImGui.get())->Win32_ProcHandler(hWnd, message, wParam, lParam);
                if (Handled)
                    return Handled;
            }
        ...
        virtual bool OnWindowCreated(HWND hWnd, LONG WindowWidth, LONG WindowHeight) override final
        {
            ...
            try
            {
                ...
                m_pImGui = ImGuiImplWin64::Create(ImGuiShizenCreateInfo{ m_pDevice, SCDesc }, m_hWnd);

                InitializeSample();
            }
            catch (...)
            {
                LOG_ERROR_MESSAGE("Failed to initialize Shizen Engine.");
                return false;
            }
    ...
    protected:
        ...
        virtual void SetFullscreenMode(const shz::DisplayModeAttribs& DisplayMode) override
        {
            if (m_bFullScreenWindow)
            {
