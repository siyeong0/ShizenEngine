...
namespace shz
{
	...
	public:
		...
		explicit AsyncTaskBase(IReferenceCounters* pRefCounters, float fPriority = 0) noexcept
			: TBase(pRefCounters)
		...
		virtual void SHZ_CALL_TYPE Cancel() override
		{
			m_bSafelyCancel.store(true);
		}

		virtual void SHZ_CALL_TYPE SetStatus(ASYNC_TASK_STATUS TaskStatus) override final
		{
#ifdef SHZ_DEBUG
		...
		virtual ASYNC_TASK_STATUS SHZ_CALL_TYPE GetStatus() const override final
		{
			return m_TaskStatus.load();
		}

		virtual void SHZ_CALL_TYPE SetPriority(float fPriority) override final
		{
			m_fPriority.store(fPriority);
		}

		virtual float SHZ_CALL_TYPE GetPriority() const override final
		{
			return m_fPriority.load();
		}

		virtual bool SHZ_CALL_TYPE IsFinished() const override final
		{
		...
		virtual void SHZ_CALL_TYPE WaitForCompletion() const override final
		{
			while (!IsFinished())
				std::this_thread::yield();
		}

		virtual void SHZ_CALL_TYPE WaitUntilRunning() const override final
		{
	...
	// For the list of parameters, see shz::IThreadPool::EnqueueTask() method.
	// The handler function must return the task status, see shz::IAsyncTask::Run() method.
		...
		float        fPriority = 0)
	{
		...
		{
		public:
			TaskImpl(IReferenceCounters* pRefCounters, float fPriority, HanlderType&& Handler)
				: AsyncTaskBase{ pRefCounters, fPriority }
				, m_Handler{ std::move(Handler) }
			{
			}

			virtual ASYNC_TASK_STATUS SHZ_CALL_TYPE Run(uint32 ThreadId) override final
			{
				return !m_bSafelyCancel.load() ? m_Handler(ThreadId) : ASYNC_TASK_STATUS_CANCELLED;
			}
