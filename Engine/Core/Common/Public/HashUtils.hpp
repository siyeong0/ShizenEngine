...
#include "Engine/RHI/Interface/BlendState.h"
...
namespace std
{
	...
	template <typename T>
	struct hash<shz::RefCntAutoPtr<T>>
	{
		size_t operator()(const shz::RefCntAutoPtr<T>& Key) const noexcept
		{
	...
	template <>
	struct hash<shz::HashMapStringKey>
	{
		size_t operator()(const shz::HashMapStringKey& Key) const noexcept
		{
			return Key.GetHash();
		}
	};
...
#define DEFINE_HASH(Type)                        \
	...
	DEFINE_HASH(shz::SamplerDesc);
	DEFINE_HASH(shz::StencilOpDesc);
	DEFINE_HASH(shz::DepthStencilStateDesc);
	DEFINE_HASH(shz::RasterizerStateDesc);
	DEFINE_HASH(shz::BlendStateDesc);
	DEFINE_HASH(shz::TextureViewDesc);
	DEFINE_HASH(shz::SampleDesc);
	DEFINE_HASH(shz::ShaderResourceVariableDesc);
	DEFINE_HASH(shz::ImmutableSamplerDesc);
	DEFINE_HASH(shz::PipelineResourceDesc);
	DEFINE_HASH(shz::PipelineResourceLayoutDesc);
	DEFINE_HASH(shz::RenderPassAttachmentDesc);
	DEFINE_HASH(shz::AttachmentReference);
	DEFINE_HASH(shz::ShadingRateAttachment);
	DEFINE_HASH(shz::SubpassDesc);
	DEFINE_HASH(shz::SubpassDependencyDesc);
	DEFINE_HASH(shz::RenderPassDesc);
	DEFINE_HASH(shz::LayoutElement);
	DEFINE_HASH(shz::InputLayoutDesc);
	DEFINE_HASH(shz::GraphicsPipelineDesc);
	DEFINE_HASH(shz::RayTracingPipelineDesc);
	DEFINE_HASH(shz::PipelineStateDesc);
	DEFINE_HASH(shz::PipelineResourceSignatureDesc);
	DEFINE_HASH(shz::ShaderDesc);
	DEFINE_HASH(shz::Version);
	DEFINE_HASH(shz::PipelineStateCreateInfo);
	DEFINE_HASH(shz::GraphicsPipelineStateCreateInfo);
	DEFINE_HASH(shz::ComputePipelineStateCreateInfo);
	DEFINE_HASH(shz::RayTracingPipelineStateCreateInfo);
