...
#define DECL_TRIVIALLY_SERIALIZABLE(Type)   \
	...
	public:
		...
		template <SerializerMode M = Mode, typename std::enable_if_t<M == SerializerMode::Measure, int> = 0>
		Serializer()
			: m_Start{ nullptr }
			, m_End{ m_Start + ~0u }
			, m_Ptr{ m_Start }
...
namespace std
{
	...
	template <>
	struct hash<shz::SerializedData>
	{
		size_t operator()(const shz::SerializedData& Data) const
		{
			return Data.GetHash();
		}
	};
