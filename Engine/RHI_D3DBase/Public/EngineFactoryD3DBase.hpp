...
#include "Engine/RHI/Public/EngineFactoryBase.hpp"
 ...
 // \file
 // Implementation of the shz::EngineFactoryD3DBase template class
...
namespace shz
{
	...
	public:
		...
		virtual void SHZ_CALL_TYPE EnumerateAdapters(
			Version MinVersion,
			uint32& NumAdapters,
		...
		virtual void SHZ_CALL_TYPE EnumerateDisplayModes(
			Version MinVersion,
			uint32 AdapterId,
			uint32 OutputId,
		...
		virtual GraphicsAdapterInfo GetGraphicsAdapterInfo(void* pd3Device, IDXGIAdapter1* pDXIAdapter) const
		{
...
#    endif
	...
	private:
		...
		bool CheckAdapterCompatibility(IDXGIAdapter1* pDXGIAdapter, D3D_FEATURE_LEVEL FeatureLevels) const;

		template <>
		bool CheckAdapterCompatibility<RENDER_DEVICE_TYPE_D3D11>(IDXGIAdapter1* pDXGIAdapter, D3D_FEATURE_LEVEL FeatureLevel) const
		{
			return CheckAdapterD3D11Compatibility(pDXGIAdapter, FeatureLevel);
		}

		template <>
		bool CheckAdapterCompatibility<RENDER_DEVICE_TYPE_D3D12>(IDXGIAdapter1* pDXGIAdapter, D3D_FEATURE_LEVEL FeatureLevel) const
		{
