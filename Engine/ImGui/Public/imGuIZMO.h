...
struct imguiGizmo
{
    shz::Quaternion qtV  = {0, 0, 0, 1}; // Quaternion value
    shz::Quaternion qtV2 = {0, 0, 0, 1}; // Quaternion value
    ...
    static ImVector<shz::float3> sphereVtx;
    static ImVector<int>              sphereTess;
    static ImVector<shz::float3> cubeVtx;
    static ImVector<shz::float3> cubeNorm;
    static ImVector<shz::float3> planeVtx;
    static ImVector<shz::float3> planeNorm;
    static ImVector<shz::float3> arrowVtx[4];
    static ImVector<shz::float3> arrowNorm[4];

    static void buildPlane(const float size, const float thickness = planeThickness)
    {
        buildPolygon(shz::float3(thickness, size, size), planeVtx, planeNorm);
    }

    static void buildCube(const float size)
    {
        buildPolygon(shz::float3(size, size, size), cubeVtx, cubeNorm);
    }

    static void buildPolygon (const shz::float3& size, ImVector<shz::float3>& vtx, ImVector<shz::float3>& norm);
    ...
    // helper functions
    ///////////////////////////////////////
    static void resizeAxesOf(const shz::float3& newSize)
    {
    ...
    ////////////////////////////////////////////////////////////////////////////
    bool getTransforms(shz::Quaternion& q, const char* label, shz::float3& dir, float size)
    {
        const float len = shz::Vector3::Length(dir);
        q = shz::Quaternion::FromAxisAngle(shz::Vector3::Normalize(shz::float3(0, -dir.z, dir.y)), acosf(dir.x/len));

        bool ret = drawFunc(label, size);
        if (ret) dir = q.RotateVector(shz::float3(1, 0, 0)) * len; //return vector with original length

        return ret;
    }
    // Vec4 (xyz axis, w angle) -> quat -> trackbalTransforms -> quat -> vec4
    ////////////////////////////////////////////////////////////////////////////
    bool getTransforms(shz::Quaternion& q, const char* label, shz::float4& axis_angle, float size)
    {
        q = shz::Quaternion::FromAxisAngle(shz::float3(axis_angle.x, axis_angle.y, axis_angle.z), axis_angle.w); //g.ConvertFromAxisAngle();
   
        bool ret = drawFunc(label, size);
        
        if (ret)q.GetAxisAngle((shz::float3&)axis_angle, axis_angle.w);
    ...
    //          imguiGizmo::resizeAxesOf(shz::float3(imguiGizmo::axesResizeFactor.x, 2.0, 2.0)); 
    ...
    // Axes reduction
    ///////////////////////////////////////
    static shz::float3 axesResizeFactor;
    static shz::float3 savedAxesResizeFactor;
...
IMGUI_API bool gizmo3D(const char*, shz::Quaternion&, float=IMGUIZMO_DEF_SIZE, const int=imguiGizmo::mode3Axes|imguiGizmo::cubeAtOrigin);
IMGUI_API bool gizmo3D(const char*, shz::float4&,      float=IMGUIZMO_DEF_SIZE, const int=imguiGizmo::mode3Axes|imguiGizmo::cubeAtOrigin);
IMGUI_API bool gizmo3D(const char*, shz::float3&,      float=IMGUIZMO_DEF_SIZE, const int=imguiGizmo::modeDirection);

IMGUI_API bool gizmo3D(const char*, shz::Quaternion&, shz::Quaternion&, float=IMGUIZMO_DEF_SIZE, const int=imguiGizmo::modeDual|imguiGizmo::cubeAtOrigin);
IMGUI_API bool gizmo3D(const char*, shz::Quaternion&, shz::float4&,      float=IMGUIZMO_DEF_SIZE, const int=imguiGizmo::modeDual|imguiGizmo::cubeAtOrigin);
IMGUI_API bool gizmo3D(const char*, shz::Quaternion&, shz::float3&,      float=IMGUIZMO_DEF_SIZE, const int=imguiGizmo::modeDual|imguiGizmo::cubeAtOrigin);
