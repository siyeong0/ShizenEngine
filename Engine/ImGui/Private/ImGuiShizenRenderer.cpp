...
#include "Engine/GraphicsTools/Public/ScopedDebugGroup.hpp"
                       ...
                       sampler Texture_sampler  [[sampler(0)]])
{
		...
		
	{
		...
		IO.BackendRendererName = "ImGuiShizenRenderer";
	...
	ImGuiShizenRenderer::~ImGuiShizenRenderer()
	{
	}

	void ImGuiShizenRenderer::NewFrame(uint32 RenderSurfaceWidth, uint32 RenderSurfaceHeight, SURFACE_TRANSFORM SurfacePreTransform)
	{
		if (!m_pPSO)
			CreateDeviceObjects();
	...
	void ImGuiShizenRenderer::EndFrame()
	{
	}

	void ImGuiShizenRenderer::InvalidateDeviceObjects()
	{
	...
	void ImGuiShizenRenderer::CreateDeviceObjects()
	{
	...
	inline TEXTURE_FORMAT ImTextureFormatToShizenFormat(ImTextureFormat format)
	{
		switch (format)
		{
	...
	void ImGuiShizenRenderer::UpdateTexture(IDeviceContext* pCtx, ImTextureData* pTexData)
	{
		...
		if (pTexData->Status == ImTextureStatus_WantCreate)
		{
			...
			Desc.Format = ImTextureFormatToShizenFormat(pTexData->Format);
	...
	void ImGuiShizenRenderer::DestroyTexture(ImTextureData* tex)
	{
	...
	void ImGuiShizenRenderer::RenderDrawData(IDeviceContext* pCtx, ImDrawData* pDrawData)
	{
		...
		// DisplayPos is (0,0) for single viewport apps.
		{
			...
			case SURFACE_TRANSFORM_ROTATE_90:
				...
				Projection = Projection * float4x4::RotationZ(-PI * 0.5f);
				break;

			case SURFACE_TRANSFORM_ROTATE_180:
				// The image content is rotated 180 degrees clockwise.
				Projection = Projection * float4x4::RotationZ(-PI * 1.0f);
				break;

			case SURFACE_TRANSFORM_ROTATE_270:
				// The image content is rotated 270 degrees clockwise.
				Projection = Projection * float4x4::RotationZ(-PI * 1.5f);
				break;
