...
#include "Engine/RHI/Interface/IFence.h"
...
namespace shz
{
	...
	struct DynamicTextureArrayCreateInfo
	{
		...
		// - Desc.Type must be shz::RESOURCE_DIM_TEX_2D_ARRAY
		// - Desc.Format must not be shz::TEX_FORMAT_UNKNOWN
		// - `Desc.Width` and `Desc.Height` must not be zero
		TextureDesc Desc;

		// The number of slices in memory page.

		// This value is only relevant when Desc.Usage == shz::USAGE_SPARSE and
	...
	public:
		...
		// \param[in] CreateInfo - Texture array create information, see shz::DynamicTextureArrayCreateInfo.
		//
	...
	private:
		void CommitResize(IRenderDevice* pDevice, IDeviceContext* pContext, bool AllowNull);
