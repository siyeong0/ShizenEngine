...
#include "Engine/GraphicsUtils/Public/VariableSizeAllocationsManager.hpp"
...
namespace shz
{
	...
	public:
		...
		virtual void Allocate(uint32 NumVertices, IVertexPoolAllocation** ppAllocation) override final
		{
			if (NumVertices == 0)
			{
			...
			VariableSizeAllocationsManager::Allocation Region;
			{
				...
				{
					size_t ExtraSize = m_ExtraVertexCount != 0 ? std::max(m_ExtraVertexCount, NumVertices) : m_Mgr.GetMaxSize();
