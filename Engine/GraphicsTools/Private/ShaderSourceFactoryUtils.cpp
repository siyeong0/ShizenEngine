...
#include "Engine/Core/Common/Public/MemoryFileStream.hpp"
...
namespace shz
{
	...
	public:
		...
		CompoundShaderSourceFactory(IReferenceCounters* pRefCounters, const CompoundShaderSourceFactoryCreateInfo& CI)
			: TBase{ pRefCounters }
		{
		...
		virtual void SHZ_CALL_TYPE CreateInputStream(const Char* Name, IFileStream** ppStream) override final
		{
			CreateInputStream2(Name, CREATE_SHADER_SOURCE_INPUT_STREAM_FLAG_NONE, ppStream);
		}

		virtual void SHZ_CALL_TYPE CreateInputStream2(
			const Char* Name,
	...
	public:
		...
		MemoryShaderSourceFactory(IReferenceCounters* pRefCounters, const MemoryShaderSourceFactoryCreateInfo& CI)
			:TBase{ pRefCounters }
		{
			if (CI.CopySources)
			{
		...
		virtual void SHZ_CALL_TYPE CreateInputStream2(const Char* Name,
			CREATE_SHADER_SOURCE_INPUT_STREAM_FLAGS Flags,
			IFileStream** ppStream) override final
		{
...
extern "C"
{
	void Shizen_CreateCompoundShaderSourceFactory(const shz::CompoundShaderSourceFactoryCreateInfo& CreateInfo,
		shz::IShaderSourceInputStreamFactory** ppFactory)
	{
		shz::CreateCompoundShaderSourceFactory(CreateInfo, ppFactory);
	}

	void Shizen_CreateMemoryShaderSourceFactory(const shz::MemoryShaderSourceFactoryCreateInfo& CreateInfo,
		shz::IShaderSourceInputStreamFactory** ppFactory)
	{
		shz::CreateMemoryShaderSourceFactory(CreateInfo, ppFactory);
	}
}
