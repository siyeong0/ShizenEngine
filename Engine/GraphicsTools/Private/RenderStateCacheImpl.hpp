...
#pragma once
 ...
 // \file
 // Definition of the shz::RenderStateCacheImpl class

#include <unordered_map>
#include <mutex>

#include "RenderStateCache.h"
#include "Engine/GraphicsArchiver/Public/SerializationDevice.h"
#include "Engine/GraphicsArchiver/Public/Archiver.h"
#include "Engine/Core/Common/Public/UniqueIdentifier.hpp"
...
namespace shz
{
	...
	public:
		RenderStateCacheImpl(IReferenceCounters* pRefCounters, const RenderStateCacheCreateInfo& CreateInfo);

		IMPLEMENT_QUERY_INTERFACE_IN_PLACE(IID_RenderStateCache, TBase);

		virtual bool SHZ_CALL_TYPE Load(const IDataBlob* pArchive,
		...
		virtual bool SHZ_CALL_TYPE WriteToBlob(uint32 ContentVersion, IDataBlob** ppBlob) override final;

		virtual bool SHZ_CALL_TYPE WriteToStream(uint32 ContentVersion, IFileStream* pStream) override final;

		virtual void SHZ_CALL_TYPE Reset() override final;

		virtual uint32 SHZ_CALL_TYPE Reload(ReloadGraphicsPipelineCallbackType ReloadGraphicsPipeline, void* pUserData) override final;

		virtual uint32 SHZ_CALL_TYPE GetContentVersion() const override final
		{
			return m_pDearchiver ? m_pDearchiver->GetContentVersion() : ~0u;
		}

		virtual uint32 SHZ_CALL_TYPE GetReloadVersion() const override final
		{
			return m_ReloadVersion;
		}

		bool CreateShaderInternal(const ShaderCreateInfo& ShaderCI, IShader** ppShader);

		template <typename CreateInfoType>
		bool CreatePipelineStateInternal(const CreateInfoType& PSOCreateInfo, IPipelineState** ppPipelineState);
	...
	private:
		...
		template <typename CreateInfoType>
		bool CreatePipelineState(const CreateInfoType& PSOCreateInfo, IPipelineState** ppPipelineState);
