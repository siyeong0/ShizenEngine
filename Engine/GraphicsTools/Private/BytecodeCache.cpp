...
#include "Engine/Core/Common/Public/Serializer.hpp"
...
namespace shz
{
	...
	public:
		BytecodeCacheImpl(IReferenceCounters* pRefCounters, const BytecodeCacheCreateInfo& CreateInfo)
			: TBase(pRefCounters)
		...
		virtual bool SHZ_CALL_TYPE Load(IDataBlob* pDataBlob) override final
		{
		...
		virtual void SHZ_CALL_TYPE RemoveBytecode(const ShaderCreateInfo& ShaderCI) override final
		{
			const XXH128Hash Hash = ComputeHash(ShaderCI);
		...
		virtual void SHZ_CALL_TYPE Clear() override final
		{
			m_HashMap.clear();
		}
...
extern "C"
{
	void CreateBytecodeCache(const shz::BytecodeCacheCreateInfo& CreateInfo, shz::IBytecodeCache** ppCache)
	{
		shz::CreateBytecodeCache(CreateInfo, ppCache);
	}
}
