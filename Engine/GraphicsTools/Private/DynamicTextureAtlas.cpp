...
#include "Engine/GraphicsUtils/Public/DynamicAtlasManager.hpp"
...
namespace shz
{
	...
	public:
		...
		DynamicTextureAtlasImpl(IReferenceCounters* pRefCounters, IRenderDevice* pDevice, const DynamicTextureAtlasCreateInfo& CreateInfo)
			...
			, m_ExtraSliceFactor{ shz::Clamp(CreateInfo.GrowthFactor, 1.f, 2.f) - 1.f }
		...
		{
			return m_DynamicTexArray ? m_DynamicTexArray->GetTexture() : m_pTexture;
		}
	...
	{
		const float2       Origin = { static_cast<float>(GetOrigin().x), static_cast<float>(GetOrigin().y) };
		const float2       Size = { static_cast<float>(GetSize().x), static_cast<float>(GetSize().y) };
