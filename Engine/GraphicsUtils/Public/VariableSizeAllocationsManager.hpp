...
namespace shz
{
	...
	private:
		...
		using TFreeBlocksByOffsetMap =
			std::map<OffsetType, FreeBlockInfo, std::less<OffsetType>, STDAllocatorRawMem<std::pair<const OffsetType, FreeBlockInfo>>>;

		// Type of the map that keeps memory blocks sorted by their sizes
		using TFreeBlocksBySizeMap =
			std::multimap<OffsetType, TFreeBlocksByOffsetMap::iterator, std::less<OffsetType>, STDAllocatorRawMem<std::pair<const OffsetType, TFreeBlocksByOffsetMap::iterator>> >;
