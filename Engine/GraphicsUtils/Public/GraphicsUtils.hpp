...
namespace shz
{
	...
	// \param [in] FilterType   - Filter type, see shz::FILTER_TYPE.
	// \param [in] bGetFullName - Whether to return string representation of the enum value.
	...
	// \param [in] AddressMode  - Texture address mode, see shz::TEXTURE_ADDRESS_MODE.
	// \param [in] bGetFullName - Whether to return string representation of the enum value.
	...
	// \param [in] ComparisonFunc - Comparison function, see shz::COMPARISON_FUNCTION.
	// \param [in] bGetFullName   - Whether to return string representation of the enum value.
	...
	// \param [in] StencilOp - Stencil operation, see shz::STENCIL_OP.
	...
	// \param [in] BlendFactor - Blend factor, see shz::BLEND_FACTOR.
	...
	// \param [in] BlendOp - Blend operation, see shz::BLEND_OPERATION.
	...
	// \param [in] FillMode - Fill mode, see shz::FILL_MODE.
	...
	// \param [in] CullMode      - Cull mode, see shz::CULL_MODE.
	...
	inline void WriteShaderMatrix(void* pDst, const float4x4& Mat, bool Transpose)
	{
		...
		{
			const float4x4 TransposedMat = Mat.Transposed();
	...
	inline void WriteShaderMatrices(void* pDst, const float4x4* pMat, size_t NumMatrices, bool Transpose)
	{
		...
		else
		{
			...
			{
				const float4x4 TransposedMat = pMat[i].Transposed();
