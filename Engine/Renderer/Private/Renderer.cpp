...
#include "Shaders/HLSL_Structures.hlsli"
	...
	bool Renderer::Initialize(const RendererCreateInfo& createInfo)
	{
		...
		// Perlin noise
		{
			AssetRef<Texture> perlinNoiseTexRef = m_pAssetManager->RegisterAsset<Texture>("C:/Dev/ShizenEngine/Assets/PerlinNoise.png");
			AssetPtr<Texture> perlinNoiseTexPtr = m_pAssetManager->Acquire(perlinNoiseTexRef);

			Texture* pPerlinNoiseTex = perlinNoiseTexPtr.Get();
			const auto& mips = pPerlinNoiseTex->GetMips();
			ASSERT(!mips.empty(), "TextureAsset has no mips.");
			...
			TextureDesc desc = {};
			desc.Name = "PerlinNoiseTexture";
			desc.Type = RESOURCE_DIM_TEX_2D;
			desc.Width = width;
			desc.Height = height;
			desc.MipLevels = static_cast<uint32>(mips.size());
			desc.ArraySize = 1;
			desc.Format = pPerlinNoiseTex->GetFormat();
			desc.Usage = USAGE_DEFAULT;
			desc.BindFlags = BIND_SHADER_RESOURCE;
			...
			AddTexture(STRING_HASH("PerlinNoiseTex"), std::move(blueNoiseTex));
			m_pPipelineStateManager->RegisterStaticTextureResource("g_PerlinNoiseTex", STRING_HASH("PerlinNoiseTex"));
		}
		// Blue noise
		{
