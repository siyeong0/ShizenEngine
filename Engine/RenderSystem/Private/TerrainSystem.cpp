...
#include "Shaders/HLSL_Structures.hlsli"
    ...
    {
        float3 Pos; // x,z in [0..ChunkQuadsPerSide]
        float2 UV;  // [0..1]
        float3 Normal;
        float3 Tangent;
    };
    ...
    static void buildGridVB(uint32 quadsPerSide, std::vector<TerrainVertex>& outVerts)
    {
        const uint32 vertsPerSide = quadsPerSide + 1;
        ...
        for (uint32 z = 0; z < vertsPerSide; ++z)
        {
            ...
            {
                const float u = float(x) / float(quadsPerSide);
                const float v = float(z) / float(quadsPerSide);

                TerrainVertex vtx = {};
                vtx.Pos = float3{ float(x), 0.0f, float(z) };
                vtx.UV = float2{ u, v };
                vtx.Normal = float3{ 0.0f, 1.0f, 0.0f }; // Not used (computed in VS)
    ...
    static void buildGridIndicesLOD_Stitched(
        uint32 quadsPerSide,
        uint32 step,
        uint8  stitchMask,
        std::vector<uint16>& outIdxU16)
    {
        outIdxU16.clear();

        ASSERT(quadsPerSide >= 2, "quadsPerSide too small.");
        ASSERT(step >= 1, "Invalid step.");
        ASSERT((quadsPerSide % step) == 0, "Step must divide quadsPerSide.");

        const uint32 vertsPerSide = quadsPerSide + 1;
        const uint32 quadsInThisLod = quadsPerSide / step;

        ASSERT(vertsPerSide * vertsPerSide <= 65535u, "Grid too large for uint16 indices. Use smaller quadsPerSide or add u32 index path.");

        auto vid = [&](uint32 gx, uint32 gz) -> uint16
            {
                ASSERT(gx <= quadsPerSide && gz <= quadsPerSide, "Grid index out of range.");
        ...
        // Step == quadsPerSide => single quad
        if (step == quadsPerSide)
        {
            outIdxU16.reserve(6);
            pushQuadStd(0, 0);
        ...
        outIdxU16.reserve(size_t(quadsInThisLod) * size_t(quadsInThisLod) * 6 + size_t(quadsInThisLod) * 12);

        // Interior (exclude outer ring)
        if (quadsInThisLod > 2)
        {
            for (uint32 qz = 1; qz < quadsInThisLod - 1; ++qz)
            {
                for (uint32 qx = 1; qx < quadsInThisLod - 1; ++qx)
                {
        ...
        if (quadsInThisLod >= 2)
        {
            if (!bStitchL)
            {
                ...
                const uint32 qzEnd = (quadsInThisLod > 1) ? (quadsInThisLod - 1u) : 0u;
    ...
    void TerrainSystem::Initialize(Renderer& renderer, AssetManager& assetManager, const CreateInfo& ci)
    {
        ...
        ASSERT(m_CI.ChunkQuadsPerSide >= 2, "ChunkQuadsPerSide too small.");
        ASSERT(IsPowerOfTwoU32(m_CI.ChunkQuadsPerSide), "ChunkQuadsPerSide must be power-of-two (16/32/64...).");
        ASSERT(m_CI.QuadSizeX > 0.f && m_CI.QuadSizeZ > 0.f, "QuadSize must be > 0.");

        m_ChunkQuadsPerSide = m_CI.ChunkQuadsPerSide;
        m_QuadSizeX = m_CI.QuadSizeX;
        m_QuadSizeZ = m_CI.QuadSizeZ;
        m_ChunkSizeXZ = float2
        {
            float(m_ChunkQuadsPerSide) * m_QuadSizeX,
            float(m_ChunkQuadsPerSide) * m_QuadSizeZ
        };

        m_NumLods = Log2U32(m_ChunkQuadsPerSide) + 1;
        ASSERT(m_NumLods <= kMaxTerrainLods, "Too many terrain LODs for this build. Increase kMaxTerrainLods.");
        ...
        // Terrain layer textures (domain masks etc.)
        ...
        // TerrainDrawConstantsBuffer (StructuredBuffer SRV, updated once per frame)
        {
            ...
            const uint32 numChunksX = uint32(std::ceil(worldSizeX / Max(1e-6f, m_ChunkSizeXZ.x)));
            const uint32 numChunksZ = uint32(std::ceil(worldSizeZ / Max(1e-6f, m_ChunkSizeXZ.y)));
        ...
        // VB (grid)
        {
            ...
            buildGridVB(m_ChunkQuadsPerSide, verts);
        ...
        for (uint32 lod = 0; lod < m_NumLods; ++lod)
        {
            ...
            ASSERT(step <= m_ChunkQuadsPerSide, "Invalid step derived from LOD.");

            for (uint32 mask = 0; mask < kNumStitchMasks; ++mask)
            {
                std::vector<uint16> idx;
                buildGridIndicesLOD_Stitched(m_ChunkQuadsPerSide, step, uint8(mask), idx);

                m_LodIndexCount[lod][mask] = uint32(idx.size());

                BufferDesc ib = {};
                std::string name =
                    "Terrain.Grid.IB.quads" + std::to_string(m_ChunkQuadsPerSide) +
        ...
        // GPU resources
        {
            // Height
            {
                ...
                sr.Stride = m_Width * sizeof(uint16); // row pitch
                sr.DepthStride = 0;
    ...
    void TerrainSystem::Cleanup()
    {
        ...
        m_ChunkQuadsPerSide = 16;
        m_QuadSizeX = 1.0f;
        m_QuadSizeZ = 1.0f;
        m_ChunkSizeXZ = float2{ 16.0f, 16.0f };
        m_NumLods = 0;
        ...
        for (uint32 lod = 0; lod < kMaxTerrainLods; ++lod)
        {
            for (uint32 mask = 0; mask < kNumStitchMasks; ++mask)
            {
    ...
    void TerrainSystem::Update(Renderer& renderer, RenderScene* pScene, const View& view)
    {
        ...
        if (m_Width == 0 || m_Height == 0 || m_NumLods == 0)
            return;
        ...
        // Update TerrainCB (dynamic) once per frame
        {
            hlsl::TerrainConstants cb = {};
            cb.WorldOriginXZ = float2{ worldOriginX, worldOriginZ };
            cb.WorldSizeXZ = float2{ worldSizeX, worldSizeZ };
            cb.ChunkSizeXZ = m_ChunkSizeXZ;
            cb.WorldSpacingXZ = float2{ m_WorldSpacingX, m_WorldSpacingZ };

            cb.HeightTexelSize = float2
            {
                1.0f / Max(1.0f, float(m_Width)),
                1.0f / Max(1.0f, float(m_Height))
            };
            ...
            cb.NormalUpBias = m_CI.bReadRChannelOnly ? 2.0f : 2.0f; // keep your default; tweak if you want
            cb.ChunkGridRes = m_ChunkQuadsPerSide;                  // NEW (shader change required)
            cb.InvChunkGridRes = 1.0f / Max(1.0f, float(m_ChunkQuadsPerSide)); // NEW
        ...
        // LOD ranges (기존 그대로; 원하면 ChunkGridRes/QuadSize 기반으로 재튜닝 가능)
        ...
        auto selectLodOnly = [&](float dist) -> uint32
            {
                // NOTE: m_NumLods와 lodRanges(5개)가 다를 수 있으므로 clamp
                uint32 lod = 0;
                if (dist < lodRanges[0].End) lod = 0;
                else if (dist < lodRanges[1].End) lod = 1;
                else if (dist < lodRanges[2].End) lod = 2;
                else if (dist < lodRanges[3].End) lod = 3;
                else lod = 4;

                return Clamp(lod, 0u, Max(0u, m_NumLods - 1));
            };

        auto morphForClampedLod = [&](float dist, uint32 lod) -> float
            {
                if (lod + 1 >= m_NumLods) return 0.0f;
        ...
        lodGrid.resize(size_t(numChunksX) * size_t(numChunksZ), uint8(Max(0u, m_NumLods - 1)));

        {
            ...
            for (uint32 cz = 0; cz < numChunksZ; ++cz)
            {
                ...
                {
                    const float chunkOriginX = worldOriginX + float(cx) * m_ChunkSizeXZ.x;
                    const float chunkOriginZ = worldOriginZ + float(cz) * m_ChunkSizeXZ.y;

                    const float remainX = worldOriginX + worldSizeX - chunkOriginX;
                    const float remainZ = worldOriginZ + worldSizeZ - chunkOriginZ;

                    const float chunkSizeX = (remainX > 0.f) ? Min(m_ChunkSizeXZ.x, remainX) : 0.f;
                    const float chunkSizeZ = (remainZ > 0.f) ? Min(m_ChunkSizeXZ.y, remainZ) : 0.f;

                    if (chunkSizeX <= 1e-6f || chunkSizeZ <= 1e-6f)
                    {
                        lodGrid[idx2D(cx, cz)] = uint8(Max(0u, m_NumLods - 1));
                        continue;
                    }
        ...
        for (uint32 cz = 0; cz < numChunksZ; ++cz)
        {
            for (uint32 cx = 0; cx < numChunksX; ++cx)
            {
                ...
                localBounds.Max = float3{ m_ChunkSizeXZ.x, yMax, m_ChunkSizeXZ.y };
                ...
                // Morph factor (현재 TODO 유지)
                const float cxw = chunkOriginX + 0.5f * m_ChunkSizeXZ.x;
                const float czw = chunkOriginZ + 0.5f * m_ChunkSizeXZ.y;
                const float dx = cam.x - cxw;
                const float dz = cam.z - czw;
                const float dist = std::sqrt(dx * dx + dz * dz);
