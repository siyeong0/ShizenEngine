...
#include "Shaders/HLSL_Structures.hlsli"
    ...
    // ---------------------------------------------------------------------
    // CPU-side batch descriptors (match HLSL struct packing: 16 bytes)
    // ---------------------------------------------------------------------
    struct InteractionBatchDescCPU final
    {
        uint32 Offset = 0; // start in Indices array
        uint32 Count = 0; // number of stamps
        uint32 _Pad0 = 0;
        uint32 _Pad1 = 0;
    };
    static_assert(sizeof(InteractionBatchDescCPU) == 16, "InteractionBatchDescCPU must be 16 bytes.");

    struct StampLocalRect final
    {
        uint32 StampIndex = 0;
        uint32 TileKey = 0; // for sorting
    ...
    void InteractionSystem::InstallPasses(Renderer& renderer, TerrainSystem& terrain)
    {
        ...
        // Batch desc buffer (Structured, dynamic CPU write)
        {
            BufferDesc bd = {};
            bd.Name = "InteractionBatchDescBuffer";
            bd.Usage = USAGE_DYNAMIC;
            bd.BindFlags = BIND_SHADER_RESOURCE;
            bd.Mode = BUFFER_MODE_STRUCTURED;
            bd.ElementByteStride = sizeof(InteractionBatchDescCPU);
            bd.Size = uint64(MAX_NUM_BATCHES) * uint64(sizeof(InteractionBatchDescCPU));
            bd.CPUAccessFlags = CPU_ACCESS_WRITE;

            renderer.AddBuffer(STRING_HASH("InteractionBatchDescBuffer"), bd);
        }

        // Batch stamp indices buffer (Structured<uint>, dynamic CPU write)
        {
            BufferDesc bd = {};
            bd.Name = "InteractionBatchStampIndices";
            bd.Usage = USAGE_DYNAMIC;
            bd.BindFlags = BIND_SHADER_RESOURCE;
            bd.Mode = BUFFER_MODE_STRUCTURED;
            bd.ElementByteStride = sizeof(uint32);
            bd.Size = uint64(MAX_NUM_INTERACTION_STAMPS) * uint64(sizeof(uint32));
            bd.CPUAccessFlags = CPU_ACCESS_WRITE;

            renderer.AddBuffer(STRING_HASH("InteractionBatchStampIndices"), bd);
        }
        ...
        renderer.AddPass(
            ...
            [&](RenderPassBuilder& b)
            {
                ...
                b.DeclareBufferSRVRead(STRING_HASH("InteractionBatchDescBuffer"));
                b.DeclareBufferSRVRead(STRING_HASH("InteractionBatchStampIndices"));
            ...
            [this, &terrain](RenderPassContext& ctx)
            {
                ...
                const float2 texelWorld =
                    worldSizeXZ / float2((float)INTERACTION_FIELD_RESOLUTION, (float)INTERACTION_FIELD_RESOLUTION);
                ...
                // ------------------------------------------------------------
                // (3) Upload InteractionConstantsCB
                // ------------------------------------------------------------
                {
                    ...
                    {
                        {
                            ...
                            dispMap->StampIndex = 0; // unused in clear
                            dispMap->_Pad = {};
                        }
                ...
                // ------------------------------------------------------------
                // (6) Build Batches on CPU + upload batch buffers
                // ------------------------------------------------------------
                uint32 batchCount = 0;
                std::vector<InteractionBatchDescCPU> batches;
                std::vector<uint32> batchStampIndices;
                std::vector<uint4> batchRects; // local rect per batch: x,y,w,h

                batches.reserve(64);
                batchStampIndices.reserve(stampCount);
                batchRects.reserve(64);

                if (stampCount > 0)
                {
                    const float2 invWorldSize =
                        float2(1.0f / worldSizeXZ.x, 1.0f / worldSizeXZ.y);
                    const float2 sizeF =
                        float2((float)INTERACTION_FIELD_RESOLUTION, (float)INTERACTION_FIELD_RESOLUTION);

                    std::vector<StampLocalRect> stampRects;
                    stampRects.reserve(stampCount);
                    ...
                    for (uint32 si = 0; si < stampCount; ++si)
                    {
                        ...
                        // bin by stamp center tile (helps batch locality)
                        const float2 centerUV = (s.CenterXZ - desiredOrigin) * invWorldSize;
                        const uint32 cx = (uint32)std::clamp<int32>((int32)std::floor(centerUV.x * sizeF.x), 0, (int32)INTERACTION_FIELD_RESOLUTION - 1);
                        const uint32 cy = (uint32)std::clamp<int32>((int32)std::floor(centerUV.y * sizeF.y), 0, (int32)INTERACTION_FIELD_RESOLUTION - 1);

                        const uint32 tx = cx / STAMP_BIN_TILE_SIZE;
                        const uint32 ty = cy / STAMP_BIN_TILE_SIZE;
                        const uint32 tileKey = (ty << 16) | tx;
                        ...
                        stampRects.push_back(r);
                    }

                    std::sort(stampRects.begin(), stampRects.end(),
                        [](const StampLocalRect& a, const StampLocalRect& b)
                        {
                            if (a.TileKey != b.TileKey) return a.TileKey < b.TileKey;
                            return a.StampIndex < b.StampIndex;
                        });

                    // Build batches: group within same tileKey, up to STAMP_BATCH_SIZE
                    uint32 i = 0;
                    while (i < (uint32)stampRects.size() && batchCount < MAX_NUM_BATCHES)
                    {
                        const uint32 thisKey = stampRects[i].TileKey;

                        // start a batch
                        const uint32 batchOffset = (uint32)batchStampIndices.size();
                        uint32 batchN = 0;

                        uint32 bx0 = 0xFFFFFFFFu, by0 = 0xFFFFFFFFu;
                        uint32 bx1 = 0, by1 = 0;

                        while (i < (uint32)stampRects.size() &&
                            stampRects[i].TileKey == thisKey &&
                            batchN < STAMP_BATCH_SIZE)
                        {
                            const StampLocalRect& sr = stampRects[i];

                            batchStampIndices.push_back(sr.StampIndex);
                            batchN++;

                            bx0 = std::min(bx0, sr.x0);
                            by0 = std::min(by0, sr.y0);
                            bx1 = std::max(bx1, sr.x1);
                            by1 = std::max(by1, sr.y1);

                            ++i;
                        }

                        // in case same tile has more than STAMP_BATCH_SIZE, continue batching same tileKey
                        // (i remains in same key, loop will start new batch with same key next time)
                        InteractionBatchDescCPU bd = {};
                        bd.Offset = batchOffset;
                        bd.Count = batchN;

                        batches.push_back(bd);

                        const uint32 rw = (bx1 > bx0) ? (bx1 - bx0) : 0u;
                        const uint32 rh = (by1 > by0) ? (by1 - by0) : 0u;
                        uint4 rect = {}; rect.x = bx0; rect.y = by0; rect.z = rw; rect.w = rh;
                    ...
                    // Upload batch descs
                    {
                        MapHelper<InteractionBatchDescCPU> batchMap(
                            pContext,
                            ctx.pRegistry->GetBuffer(STRING_HASH("InteractionBatchDescBuffer")),
                            MAP_WRITE,
                            MAP_FLAG_DISCARD);

                        for (uint32 bi = 0; bi < batchCount; ++bi)
                        {
                            batchMap[bi] = batches[bi];
                        }
                    }

                    // Upload batch stamp indices
                    {
                        MapHelper<uint32> idxMap(
                            pContext,
                            ctx.pRegistry->GetBuffer(STRING_HASH("InteractionBatchStampIndices")),
                            MAP_WRITE,
                            MAP_FLAG_DISCARD);

                        const uint32 n = (uint32)batchStampIndices.size();
                        for (uint32 k = 0; k < n; ++k)
                        {
                            idxMap[k] = batchStampIndices[k];
                        }
                    }
                }

                // ------------------------------------------------------------
                // (7) Apply batches (per-batch rect dispatch)
                // ------------------------------------------------------------
                if (batchCount > 0)
                {
                    ...
                    for (uint32 bi = 0; bi < batchCount; ++bi)
                    {
                        ...
                        // per-dispatch CB (StampIndex is repurposed as BatchIndex)
                        {
                            MapHelper<hlsl::InteractionDispatch> dispMap(
                                pContext,
                                ctx.pRegistry->GetBuffer(STRING_HASH("InteractionDispatchCB")),
                            ...
                            dispMap->StampIndex = bi; // BatchIndex
                            dispMap->_Pad = {};
                        }
                ...
                [this, &renderer]()
            {
                ...
                // ============================================================
                // (3) ApplyBatch PSO (was per-stamp; now per-batch)
                // ============================================================
                {
                    ShaderCreateInfo csCI = {};
                    csCI.SourceLanguage = SHADER_SOURCE_LANGUAGE_HLSL;
                    csCI.EntryPoint = m_ApplyEntry.c_str();
                    csCI.Desc.Name = "InteractionApplyBatchRectCS";
                    csCI.Desc.ShaderType = SHADER_TYPE_COMPUTE;
                    csCI.Desc.UseCombinedTextureSamplers = false;
                    csCI.FilePath = m_InteractionCS.c_str();
                    ...
                    RefCntAutoPtr<IShader> cs;
                    renderer.CreateShader(csCI, &cs);
                    ASSERT(cs, "InteractionApplyBatchRectCS compile failed.");

                    ComputePipelineStateCreateInfo psoCI = {};
                    psoCI.PSODesc.Name = "PSO_InteractionApplyBatchRect";
                    psoCI.PSODesc.PipelineType = PIPELINE_TYPE_COMPUTE;
                    ...
                    ShaderResourceVariableDesc vars[] =
                    {
                        ...
                        { SHADER_TYPE_COMPUTE, "g_Batches",                SHADER_RESOURCE_VARIABLE_TYPE_MUTABLE },
                        { SHADER_TYPE_COMPUTE, "g_BatchStampIndices",      SHADER_RESOURCE_VARIABLE_TYPE_MUTABLE },
                        { SHADER_TYPE_COMPUTE, "INTERACTION_CONSTANTS",     SHADER_RESOURCE_VARIABLE_TYPE_MUTABLE },
                        { SHADER_TYPE_COMPUTE, "INTERACTION_DISPATCH",      SHADER_RESOURCE_VARIABLE_TYPE_MUTABLE },
                    };
                    rl.Variables = vars;
                    ...
                    ASSERT(m_pApplyStampCSO, "AcquireCompute(InteractionApplyBatch) failed.");

                    m_pApplyStampCSO->CreateShaderResourceBinding(&m_pApplyStampSRB, true);
                    ASSERT(m_pApplyStampSRB, "InteractionApplyBatch SRB create failed.");
                    ...
                    if (auto* var = m_pApplyStampSRB->GetVariableByName(SHADER_TYPE_COMPUTE, "g_Batches"))
                    {
                        var->Set(renderer.GetBufferSRV(STRING_HASH("InteractionBatchDescBuffer")));
                    }

                    if (auto* var = m_pApplyStampSRB->GetVariableByName(SHADER_TYPE_COMPUTE, "g_BatchStampIndices"))
                    {
                        var->Set(renderer.GetBufferSRV(STRING_HASH("InteractionBatchStampIndices")));
                    }
                }
            });
    }
