...
namespace shz
{
	...
	static bool LoadImageSTB(
		const void* pSrcImage,
		size_t      ImageSize,
	...
	Image::Image(IReferenceCounters* pRefCounters, const void* pSrcData, size_t SrcDataSize, const ImageLoadInfo& LoadInfo)
		: TBase{ pRefCounters }
	...
	void Image::CreateFromMemory(const void* pSrcData, size_t SrcDataSize, const ImageLoadInfo& LoadInfo, Image** ppImage)
	{
		*ppImage = MakeNewRCObj<Image>()(pSrcData, SrcDataSize, LoadInfo);
		(*ppImage)->AddRef();
	}

	Image::Image(IReferenceCounters* pRefCounters, const ImageDesc& Desc, RefCntAutoPtr<IDataBlob> pPixels)
		: TBase{ pRefCounters }
		, m_Desc{ Desc }
	...
	void Image::CreateFromPixels(const ImageDesc& Desc, RefCntAutoPtr<IDataBlob> pPixels, Image** ppImage)
	{
	...
	std::vector<uint8> Image::ConvertImageData(
		uint32         Width,
		uint32         Height,
		const uint8* pData,
		uint32         Stride,
	...
	IMAGE_FILE_FORMAT CreateImageFromFile(const Char* FilePath, Image** ppImage, IDataBlob** ppRawData)
	{
