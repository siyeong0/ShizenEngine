...
namespace shz
{
	...
	DECODE_JPEG_RESULT DecodeJpeg(
		const void* pSrcJpegBits,
		size_t      JpegDataSize,
		...
		ImageDesc* pDstImgDesc)
	{
		...
		const unsigned char* pSrcPtr = reinterpret_cast<const unsigned char*>(pSrcJpegBits);
		...
		if (pDstPixels != NULL)
		{
			...
			while (cinfo.output_scanline < cinfo.output_height)
			{
				...
				uint8* pScanline0 = reinterpret_cast<uint8*>(pDstPixels->GetDataPtr(0));
