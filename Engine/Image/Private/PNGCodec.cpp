...
namespace shz
{
	...
	DECODE_PNG_RESULT DecodePng(const void* pSrcPngBits,size_t PngDataSize,IDataBlob* pDstPixels,ImageDesc* pDstImgDesc)
	{
		...
		ReadState.pPngBits = reinterpret_cast<const IDataBlob*>(pSrcPngBits);
		ReadState.Offset = 0;
		...
		if (pDstPixels != NULL)
		{
			...
			png_bytep pRow0 = reinterpret_cast<png_bytep>(pDstPixels->GetDataPtr(0));
	...
	static void PngWriteCallback(png_structp png_ptr, png_bytep data, png_size_t length)
	{
		...
		uint8* pBytes = reinterpret_cast<uint8*>(pEncodedData->GetDataPtr(0));
		memcpy(pBytes + PrevSize, data, length);
	}

	ENCODE_PNG_RESULT EncodePng(
		const uint8* pSrcPixels,
		uint32       Width,
		uint32       Height,
