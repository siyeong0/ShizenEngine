...
#pragma once
 ...
 // \file
 // Declaration of shz::CommandQueueD3D12Impl class

#include <mutex>
#include <atomic>

#include "ICommandQueueD3D12.h"
#include "Engine/Core/Common/Public/ObjectBase.hpp"

namespace shz
{

	// Implementation of the shz::ICommandQueueD3D12 interface
	...
	public:
		...
		virtual uint64 SHZ_CALL_TYPE GetNextFenceValue() const override final { return m_NextFenceValue.load(); }

		// Implementation of ICommandQueue::WaitForIdle().
		virtual uint64 SHZ_CALL_TYPE WaitForIdle() override final;

		// Implementation of ICommandQueue::GetCompletedFenceValue().
		virtual uint64 SHZ_CALL_TYPE GetCompletedFenceValue() override final;

		// Implementation of ICommandQueueD3D12::Submit().
		virtual uint64 SHZ_CALL_TYPE Submit(uint32 NumCommandLists, ID3D12CommandList* const* ppCommandLists) override final;

		// Implementation of ICommandQueueD3D12::GetD3D12CommandQueue().
		virtual ID3D12CommandQueue* SHZ_CALL_TYPE GetD3D12CommandQueue() override final { return m_pd3d12CmdQueue; }

		// Implementation of ICommandQueueD3D12::EnqueueSignal().
		virtual void SHZ_CALL_TYPE EnqueueSignal(ID3D12Fence* pFence, uint64 Value) override final;

		// Implementation of ICommandQueueD3D12::WaitFence().
		virtual void SHZ_CALL_TYPE WaitFence(ID3D12Fence* pFence, uint64 Value) override final;

		// Implementation of ICommandQueueD3D12::GetD3D12CommandQueueDesc().
		virtual const D3D12_COMMAND_QUEUE_DESC& SHZ_CALL_TYPE GetD3D12CommandQueueDesc() const override final
		{
			return m_d3d12CmdQueueDesc;
		}

		// Implementation of ICommandQueueD3D12::UpdateTileMappings().
		virtual void SHZ_CALL_TYPE UpdateTileMappings(ResourceTileMappingsD3D12* pMappings, uint32 Count) override final;
