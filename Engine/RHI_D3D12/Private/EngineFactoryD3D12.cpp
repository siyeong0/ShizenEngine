...
#include "Engine/RHI_D3DBase/Public/EngineFactoryD3DBase.hpp"
#include "Engine/Core/Common/Public/StringTools.h"
#include "Engine/RHI/Public/EngineFactoryBase.hpp"
...
namespace shz
{
	...
	public:
		...
		bool SHZ_CALL_TYPE LoadD3D12(const char* DllName) override final;

		virtual void SHZ_CALL_TYPE CreateDeviceAndContextsD3D12(const EngineD3D12CreateInfo& EngineCI,
			IRenderDevice** ppDevice,
			IDeviceContext** ppContexts) override final;

		virtual void SHZ_CALL_TYPE CreateCommandQueueD3D12(void* pd3d12NativeDevice,
			void* pd3d12NativeCommandQueue,
			IMemoryAllocator* pRawMemAllocator,
			ICommandQueueD3D12** ppCommandQueue) override final;

		virtual void SHZ_CALL_TYPE AttachToD3D12Device(void* pd3d12NativeDevice,
		...
		virtual void SHZ_CALL_TYPE CreateSwapChainD3D12(IRenderDevice* pDevice,
		...
		virtual void SHZ_CALL_TYPE EnumerateAdapters(Version              MinFeatureLevel,
			uint32& NumAdapters,
			GraphicsAdapterInfo* Adapters) const override final;

		virtual void SHZ_CALL_TYPE EnumerateDisplayModes(Version             MinFeatureLevel,
			uint32              AdapterId,
			uint32              OutputId,
		...
		virtual void SHZ_CALL_TYPE CreateDearchiver(const DearchiverCreateInfo& CreateInfo,
	...
	{
		if (EngineCI.EngineAPIVersion != SHZ_API_VERSION)
		{
			LOG_ERROR_MESSAGE("Shizen Engine runtime (", SHZ_API_VERSION, ") is not compatible with the client API version (", EngineCI.EngineAPIVersion, ")");
			return;
		}
		...
		ICommandQueueD3D12** ppCommandQueue)
	{
		...
		*ppCommandQueue = nullptr;
		...
		try
		{
			...
			RefCntAutoPtr<CommandQueueD3D12Impl> pCmdQueueD3D12 = shz::CreateCommandQueueD3D12(d3d12Device, d3d12CmdQueue, L"Fence for user-provided command queue");
		...
		IDeviceContext** ppContexts)
	{
		if (EngineCI.EngineAPIVersion != SHZ_API_VERSION)
		{
			LOG_ERROR_MESSAGE("Diligent Engine runtime (", SHZ_API_VERSION, ") is not compatible with the client API version (", EngineCI.EngineAPIVersion, ")");
			return;
		}
...
extern "C"
{
	shz::IEngineFactoryD3D12* Shizen_GetEngineFactoryD3D12()
	{
		return shz::GetEngineFactoryD3D12();
	}
}
