...
#include "Platforms/Common/NativeWindow.h"
#include "Engine/Core/Common/Public/StringTools.h"
#include "Engine/Core/Common/Public/ThreadPool.h"
#include "APIInfo.h"
#include "Constants.h"
...
namespace shz
{
	...
	enum USAGE : uint8
	{
		...
		// the adapter info (see shz::IRenderDevice::GetAdapterInfo().Memory and shz::AdapterMemoryInfo).
		...
		// (see shz::AdapterMemoryInfo::UnifiedMemoryCPUAccess).
		USAGE_UNIFIED,
	...
	struct DepthStencilClearValue
	{
		...
		constexpr DepthStencilClearValue(float32 _Depth, uint8 _Stencil) noexcept
			: Depth{ _Depth }
			, Stencil{ _Stencil }
		{
		}
	...
	struct SwapChainDesc
	{
		...
		// Back buffer format. Default value is shz::TEX_FORMAT_RGBA8_UNORM_SRGB
		TEXTURE_FORMAT ColorBufferFormat = TEX_FORMAT_RGBA8_UNORM_SRGB;

		// Depth buffer format. Default value is shz::TEX_FORMAT_D32_FLOAT.
		// Use shz::TEX_FORMAT_UNKNOWN to create the swap chain without depth buffer.
		TEXTURE_FORMAT DepthBufferFormat = TEX_FORMAT_D32_FLOAT;

		// Swap chain usage flags. Default value is shz::SWAP_CHAIN_USAGE_RENDER_TARGET
	...
	enum QUERY_TYPE
	{
		...
		// and IDeviceContext::EndQuery. IQuery::GetData fills a shz::QueryDataOcclusion struct.
		...
		// IQuery::GetData fills a shz::QueryDataBinaryOcclusion struct.
		QUERY_TYPE_BINARY_OCCLUSION,

		// Gets the GPU timestamp corresponding to IDeviceContext::EndQuery call. For this query
		// type IDeviceContext::BeginQuery is disabled. IQuery::GetData fills a shz::QueryDataTimestamp struct.
		QUERY_TYPE_TIMESTAMP,

		// Gets pipeline statistics, such as the number of pixel shader invocations in between IDeviceContext::BeginQuery
		// and IDeviceContext::EndQuery. IQuery::GetData fills a shz::QueryDataPipelineStatistics struct.
		QUERY_TYPE_PIPELINE_STATISTICS,

		// Gets the number of high-frequency counter ticks between IDeviceContext::BeginQuery and
		// IDeviceContext::EndQuery calls. IQuery::GetData fills a shz::QueryDataDuration struct.
		QUERY_TYPE_DURATION,
	...
	enum DEVICE_FEATURE_STATE : uint8
	{
	...
	struct DeviceFeatures
	{
		...
		// See shz::RayTracingProperties for more information.
		...
		// Indicates if device supports occlusion queries (see shz::QUERY_TYPE_OCCLUSION).
		DEVICE_FEATURE_STATE OcclusionQueries = DEVICE_FEATURE_STATE_DISABLED;

		// Indicates if device supports binary occlusion queries (see shz::QUERY_TYPE_BINARY_OCCLUSION).
		DEVICE_FEATURE_STATE BinaryOcclusionQueries = DEVICE_FEATURE_STATE_DISABLED;

		// Indicates if device supports timestamp queries (see shz::QUERY_TYPE_TIMESTAMP).
		DEVICE_FEATURE_STATE TimestampQueries = DEVICE_FEATURE_STATE_DISABLED;

		// Indicates if device supports pipeline statistics queries (see shz::QUERY_TYPE_PIPELINE_STATISTICS).
		DEVICE_FEATURE_STATE PipelineStatisticsQueries = DEVICE_FEATURE_STATE_DISABLED;

		// Indicates if device supports duration queries (see shz::QUERY_TYPE_DURATION).
...
#undef HandleFeature
	...
	enum RAY_TRACING_CAP_FLAGS : uint8
	{
	...
	struct RayTracingProperties
	{
		...
		uint32 ShaderGroupHandleSize = 0; ///< For internal use
		uint32 MaxShaderRecordStride = 0; ///< For internal use
		uint32 ShaderGroupBaseAlignment = 0; ///< For internal use
		...
		// Ray tracing capability flags, see shz::RAY_TRACING_CAP_FLAGS.
	...
	{
		// Device type. See shz::RENDER_DEVICE_TYPE.
		...
		// Enabled device features. See shz::DeviceFeatures.
		...
		// Maximum supported version for each shader language, see shz::RenderDeviceShaderVersionInfo.
	...
	struct AdapterMemoryInfo
	{
		...
		// If supported, it will be combination of the following flags: shz::BIND_RENDER_TARGET,
		// shz::BIND_DEPTH_STENCIL, shz::BIND_INPUT_ATTACHMENT.
	...
	enum SHADING_RATE_FORMAT : uint8
	{
		// Variable rate shading is not supported.
		SHADING_RATE_FORMAT_UNKNOWN = 0,

		// Single-channel 8-bit surface that contains shz::SHADING_RATE values.
	...
	enum SHADING_RATE : uint8
	{
		...
		SHADING_RATE_1X1 = ((AXIS_SHADING_RATE_1X << SHADING_RATE_X_SHIFT) | AXIS_SHADING_RATE_1X),

		// Specifies default horizontal rate and 1/2 vertical shading rate.
		SHADING_RATE_1X2 = ((AXIS_SHADING_RATE_1X << SHADING_RATE_X_SHIFT) | AXIS_SHADING_RATE_2X),

		// Specifies default horizontal rate and 1/4 vertical shading rate.
		SHADING_RATE_1X4 = ((AXIS_SHADING_RATE_1X << SHADING_RATE_X_SHIFT) | AXIS_SHADING_RATE_4X),

		// Specifies 1/2 horizontal shading rate and default vertical rate.
		SHADING_RATE_2X1 = ((AXIS_SHADING_RATE_2X << SHADING_RATE_X_SHIFT) | AXIS_SHADING_RATE_1X),

		// Specifies 1/2 horizontal and 1/2 vertical shading rate.
		SHADING_RATE_2X2 = ((AXIS_SHADING_RATE_2X << SHADING_RATE_X_SHIFT) | AXIS_SHADING_RATE_2X),

		// Specifies 1/2 horizontal and 1/4 vertical shading rate.
		SHADING_RATE_2X4 = ((AXIS_SHADING_RATE_2X << SHADING_RATE_X_SHIFT) | AXIS_SHADING_RATE_4X),

		// Specifies 1/4 horizontal and default vertical rate.
		SHADING_RATE_4X1 = ((AXIS_SHADING_RATE_4X << SHADING_RATE_X_SHIFT) | AXIS_SHADING_RATE_1X),

		// Specifies 1/4 horizontal and 1/2 vertical rate.
		SHADING_RATE_4X2 = ((AXIS_SHADING_RATE_4X << SHADING_RATE_X_SHIFT) | AXIS_SHADING_RATE_2X),

		// Specifies 1/4 horizontal and 1/4 vertical shading rate.
		SHADING_RATE_4X4 = ((AXIS_SHADING_RATE_4X << SHADING_RATE_X_SHIFT) | AXIS_SHADING_RATE_4X),
	...
	enum SHADING_RATE_CAP_FLAGS : uint16
	{
		...
		// must be created with shz::MISC_TEXTURE_FLAG_SUBSAMPLED flag.
		// Intermediate targets must be scaled to the final resolution in a separate pass.
		// Intermediate targets can only be sampled with an immutable sampler created with shz::SAMPLER_FLAG_SUBSAMPLED flag.
	...
	enum SHADING_RATE_TEXTURE_ACCESS : uint8
	{
	...
	struct ShadingRateProperties
	{
		...
		ShadingRateMode        ShadingRates[MAX_SHADING_RATES] = {};

		// The number of valid elements in ShadingRates array.
		uint8                  NumShadingRates = 0;

		// Shading rate capability flags, see shz::SHADING_RATE_CAP_FLAGS.
		SHADING_RATE_CAP_FLAGS CapFlags = SHADING_RATE_CAP_FLAG_NONE;

		// Combination of all supported shading rate combiners (see shz::SHADING_RATE_COMBINER).
		SHADING_RATE_COMBINER  Combiners = SHADING_RATE_COMBINER_PASSTHROUGH;

		// Indicates which shading rate texture format is used by this device (see shz::SHADING_RATE_FORMAT).
		SHADING_RATE_FORMAT    Format = SHADING_RATE_FORMAT_UNKNOWN;

		// Shading rate texture access type (see shz::SHADING_RATE_TEXTURE_ACCESS).
	...
	{
		// Draw command capability flags, see shz::DRAW_COMMAND_CAP_FLAGS.
	...
	struct SparseResourceProperties
	{
		...
		// Sparse resource capability flags, see shz::SPARSE_RESOURCE_CAP_FLAGS.
	...
	{
		// Indicates which type of commands are supported by this queue, see shz::COMMAND_QUEUE_TYPE.
	...
	struct GraphicsAdapterInfo
	{
		...
		// Adapter type, see shz::ADAPTER_TYPE.
		ADAPTER_TYPE   Type = ADAPTER_TYPE_UNKNOWN;

		// Adapter vendor, see shz::ADAPTER_VENDOR.
		...
		// Device memory information, see shz::AdapterMemoryInfo.
		AdapterMemoryInfo Memory;

		// Ray tracing properties, see shz::RayTracingProperties.
		RayTracingProperties RayTracing;

		// Wave operation properties, see shz::WaveOpProperties.
		WaveOpProperties WaveOp;

		// Buffer properties, see shz::BufferProperties.
		BufferProperties Buffer;

		// Texture properties, see shz::TextureProperties.
		TextureProperties Texture;

		// Sampler properties, see shz::SamplerProperties.
		SamplerProperties Sampler;

		// Mesh shader properties, see shz::MeshShaderProperties.
		MeshShaderProperties MeshShader;

		// Shading rate properties, see shz::ShadingRateProperties.
		ShadingRateProperties ShadingRate;

		// Compute shader properties, see shz::ComputeShaderProperties.
		ComputeShaderProperties ComputeShader;

		// Draw command properties, see shz::DrawCommandProperties.
		DrawCommandProperties DrawCommand;

		// Sparse resource properties, see shz::SparseResourceProperties.
		SparseResourceProperties SparseResources;

		// Supported device features, see shz::DeviceFeatures.
		...
		// An array of NumQueues command queues supported by this device. See shz::CommandQueueInfo.
		CommandQueueInfo  Queues[MAX_ADAPTER_QUEUES] = {};
	...
	struct ImmediateContextCreateInfo
	{
		...
		// Priority of the software queue created by the context, see shz::QUEUE_PRIORITY.
		...
		constexpr ImmediateContextCreateInfo(
			const Char* _Name,
			uint8 _QueueId,
	...
	struct EngineCreateInfo
	{
		...
		int32                    EngineAPIVersion = SHZ_API_VERSION;
		...
		// device contexts to create. See shz::ImmediateContextCreateInfo.
		...
		// Validation options, see shz::VALIDATION_FLAGS.
		...
		// Sets the validation options corresponding to the specified level, see shz::VALIDATION_LEVEL.
	...
	enum D3D11_VALIDATION_FLAGS : uint32
	{
		...
		// This option is enabled in validation level 2 (see shz::VALIDATION_LEVEL).
		//
	...
	{
		// Direct3D11-specific validation options, see shz::D3D11_VALIDATION_FLAGS.
	...
	enum D3D12_VALIDATION_FLAGS : uint32
	{
	...
	struct EngineD3D12CreateInfo : public EngineCreateInfo 
	{
		...
		// Direct3D12-specific validation options, see shz::D3D12_VALIDATION_FLAGS.
	...
	struct VulkanDescriptorPoolSize
	{
		...
		uint32 NumInputAttachmentDescriptors = 0;
		uint32 NumAccelStructDescriptors = 0;

		constexpr VulkanDescriptorPoolSize() noexcept {}

		constexpr VulkanDescriptorPoolSize(
			uint32 _MaxDescriptorSets,
...
#undef HandleFeature
	...
	{
		// Vulkan-specific device features, see shz::DeviceFeaturesVk.
	...
	struct EngineWebGPUCreateInfo : public EngineCreateInfo 
	{
	...
	struct TextureFormatAttribs
	{
		...
		// Texture format, see shz::TEXTURE_FORMAT for a list of supported texture formats
		TEXTURE_FORMAT Format = TEX_FORMAT_UNKNOWN;
		...
		// Component type, see shz::COMPONENT_TYPE for details.
		...
		constexpr TextureFormatAttribs(
			const Char* _Name,
	...
	struct TextureFormatInfoExt : public TextureFormatInfo 
	{
		...
		// see shz::RESOURCE_DIMENSION_SUPPORT.
	...
	enum SPARSE_TEXTURE_FLAGS : uint8
	{
	...
	struct SparseTextureFormatInfo
	{
		...
		// Sparse texture flags, see shz::SPARSE_TEXTURE_FLAGS.
