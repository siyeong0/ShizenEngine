...
#pragma once
 ...
 // \file
 // Definition of the shz::IPipelineState interface and related data structures
...
namespace shz
{
	...
	enum SHADER_VARIABLE_FLAGS : uint8
	{
		...
		// variable. Applies to shz::SHADER_RESOURCE_TYPE_CONSTANT_BUFFER,
		// shz::SHADER_RESOURCE_TYPE_BUFFER_UAV, shz::SHADER_RESOURCE_TYPE_BUFFER_SRV resources.
		//
		// \remarks    This flag directly translates to the shz::PIPELINE_RESOURCE_FLAG_NO_DYNAMIC_BUFFERS
	...
	struct ShaderResourceVariableDesc
	{
		...
		// Shader variable type. See shz::SHADER_RESOURCE_VARIABLE_TYPE for a list of allowed types
		...
		constexpr ShaderResourceVariableDesc(
	...
	enum PIPELINE_SHADING_RATE_FLAGS : uint8
	{
	...
	struct GraphicsPipelineDesc
	{
		...
		// All formats must be shz::TEX_FORMAT_UNKNOWN when `pRenderPass` is not `null`.
		TEXTURE_FORMAT RTVFormats[MAX_RENDER_TARGETS] = {};

		// Depth-stencil format.

		// Must be shz::TEX_FORMAT_UNKNOWN when `pRenderPass` is not `null`.
		...
		// and all RTV formats as well as DSV format must be shz::TEX_FORMAT_UNKNOWN.
	...
	struct RayTracingGeneralShaderGroup
	{
		...
		// Shader type must be shz::SHADER_TYPE_RAY_GEN,
		// shz::SHADER_TYPE_RAY_MISS or shz::SHADER_TYPE_CALLABLE.
		...
		constexpr RayTracingGeneralShaderGroup(const Char* _Name, IShader* _pShader) noexcept
			: Name(_Name)
			, pShader(_pShader)
		{
		}
	...
	struct RayTracingTriangleHitShaderGroup
	{
		...
		// The shader type must be shz::SHADER_TYPE_RAY_CLOSEST_HIT.
		IShader* pClosestHitShader = nullptr;

		// Any-hit shader. Can be null.

		// The shader type must be shz::SHADER_TYPE_RAY_ANY_HIT.
		IShader* pAnyHitShader = nullptr; // can be null

		constexpr RayTracingTriangleHitShaderGroup() noexcept
		{
		}

		constexpr RayTracingTriangleHitShaderGroup(
			const Char* _Name,
	...
	struct RayTracingProceduralHitShaderGroup
	{
		...
		// The shader type must be shz::SHADER_TYPE_RAY_INTERSECTION.
		...
		constexpr RayTracingProceduralHitShaderGroup(
			const Char* _Name,
	...
	struct PipelineStateCreateInfo
	{
		...
		// Pipeline state creation flags, see shz::PSO_CREATE_FLAGS.
	...
	struct RayTracingPipelineStateCreateInfo : public PipelineStateCreateInfo
	{
	...
	struct TilePipelineStateCreateInfo : public PipelineStateCreateInfo
	{
		// Tile pipeline description, see shz::TilePipelineDesc.
	...
	enum PIPELINE_STATE_STATUS : uint32
	{
		// Initial state.
	...
	struct SHZ_INTERFACE IPipelineState : public IDeviceObject
	{
		...
		// Returns the pipeline state status, see shz::PIPELINE_STATE_STATUS.
