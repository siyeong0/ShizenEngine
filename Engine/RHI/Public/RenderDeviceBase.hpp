...
#pragma once
 ...
 // \file
 // Implementation of the shz::RenderDeviceBase template class and related structures
...
#include "Engine/Core/Common/Public/ObjectsRegistry.hpp"
#include "Engine/Core/Common/Public/HashUtils.hpp"
#include "Engine/Core/Common/Public/ObjectBase.hpp"
#include "Engine/Core/Memory/Public/FixedBlockMemoryAllocator.hpp"
...
#include "Engine/Core/Common/Public/SpinLock.hpp"
...
namespace shz
{
	...
	DeviceFeaturesVk EnableDeviceFeaturesVk(const DeviceFeaturesVk& SupportedFeatures, const DeviceFeaturesVk& RequestedFeatures) noexcept(false);

	// Checks sparse texture format support and returns the component type
	COMPONENT_TYPE CheckSparseTextureFormatSupport(
	...
	public:
		...
		// \param EngineCI        - Engine create info struct, see shz::EngineCreateInfo.
		// \param AdapterInfo     - Graphics adapter info, see shz::AdapterInfo.
		//
		...
		// virtual calls
		inline virtual ReferenceCounterValueType SHZ_CALL_TYPE Release() override final
		{
		...
		virtual const RenderDeviceInfo& SHZ_CALL_TYPE GetDeviceInfo() const override final
		{
			return m_DeviceInfo;
		}

		// Implementation of IRenderDevice::GetAdapterInfo().
		virtual const GraphicsAdapterInfo& SHZ_CALL_TYPE GetAdapterInfo() const override final
		{
		...
		virtual IEngineFactory* SHZ_CALL_TYPE GetEngineFactory() const override final
		{
		...
		{
			return Ctx < m_wpImmediateContexts.size() ? m_wpImmediateContexts[Ctx].Lock() : RefCntAutoPtr<DeviceContextImplType>{};
		}
		RefCntAutoPtr<DeviceContextImplType> GetDeferredContext(size_t Ctx)
		{
			std::lock_guard<std::mutex> Guard{ m_DeferredCtxMtx };
			return Ctx < m_wpDeferredContexts.size() ? m_wpDeferredContexts[Ctx].Lock() : RefCntAutoPtr<DeviceContextImplType>{};
		}
		...
		virtual IThreadPool* SHZ_CALL_TYPE GetShaderCompilationThreadPool() const override final
		{
