...
#pragma once
 ...
 // \file
 // Implementation of the shz::BufferBase template class
...
namespace shz
{
	...
	void ValidateAndCorrectBufferViewDesc(
	...
	public:
		...
		virtual IBufferView* SHZ_CALL_TYPE GetDefaultView(BUFFER_VIEW_TYPE ViewType) override
		{
			switch (ViewType)
			{
		...
		//
		// - Creates default shader resource view addressing the entire buffer if shz::BIND_SHADER_RESOURCE flag is set
		// - Creates default unordered access view addressing the entire buffer if shz::BIND_UNORDERED_ACCESS flag is set
		//
		...
		virtual MEMORY_PROPERTIES SHZ_CALL_TYPE GetMemoryProperties() const override final
		{
			return this->m_MemoryProperties;
		}

		virtual void SHZ_CALL_TYPE FlushMappedRange(uint64 StartOffset,
			uint64 Size) override
		{
			DvpVerifyFlushMappedRangeArguments(StartOffset, Size);
		}

		virtual void SHZ_CALL_TYPE InvalidateMappedRange(uint64 StartOffset,
			uint64 Size) override
		{
