...
#pragma once
 ...
 // \file
 // Implementation of the shz::ShaderBase template class
...
#    define RESOURCE_VALIDATION_FAILURE LOG_ERROR_MESSAGE
	...
	bool VerifyResourceBinding(
		const char* ExpectedResourceTypeName,
	...
	bool VerifyConstantBufferBinding(
		const PipelineResourceDesc& ResDesc,
		const BindResourceInfo& BindInfo,
	...
	bool ValidateResourceViewDimension(
		const char* ResName,
		uint32                     ArraySize,
		uint32                     ArrayInd,
	...
	template <typename ResourceViewImplType, typename ViewTypeEnumType>
	bool VerifyResourceViewBinding(
		const PipelineResourceDesc& ResDesc,
		const BindResourceInfo& BindInfo,
	...
	bool VerifyTLASResourceBinding(
		const PipelineResourceDesc& ResDesc,
		const BindResourceInfo& BindInfo,
	...
	bool VerifyDynamicBufferOffset(
		const PipelineResourceDesc& ResDesc,
...
#undef RESOURCE_VALIDATION_FAILURE
	...
	struct ShaderVariableBase : public ResourceVariableBaseInterface
	{
		...
		virtual void SHZ_CALL_TYPE QueryInterface(const INTERFACE_ID& IID, IObject** ppInterface) override
		{
		...
		virtual void SHZ_CALL_TYPE Set(IDeviceObject* pObject, SET_SHADER_RESOURCE_FLAGS Flags) override final
		{
			static_cast<ThisImplType*>(this)->BindResource(BindResourceInfo{ pObject, Flags });
		}

		virtual void SHZ_CALL_TYPE SetArray(
		...
		virtual void SHZ_CALL_TYPE SetBufferRange(
		...
		virtual void SHZ_CALL_TYPE SetBufferOffset(uint32 Offset, uint32 ArrayIndex) override final
		{
#ifdef SHZ_DEBUG
			{
				const PipelineResourceDesc& Desc = GetDesc();
		...
		virtual SHADER_RESOURCE_VARIABLE_TYPE SHZ_CALL_TYPE GetType() const override final
		{
			return GetDesc().VarType;
		}

		virtual void SHZ_CALL_TYPE GetResourceDesc(ShaderResourceDesc& ResourceDesc) const override final
		{
			const PipelineResourceDesc& Desc = GetDesc();
		...
		virtual uint32 SHZ_CALL_TYPE GetIndex() const override final
		{
	...
	protected:
		...
		ShaderVariableManagerBase(IObject& Owner, ShaderResourceCacheType& ResourceCache) noexcept
			: m_Owner{ Owner }
