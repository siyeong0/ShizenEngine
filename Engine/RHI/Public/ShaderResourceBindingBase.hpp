...
#pragma once
 ...
 // \file
 // Implementation of the shz::ShaderResourceBindingBase template class
...
#include "Engine/RHI/Interface/Constants.h"
...
namespace shz
{
	...
	public:
		...
		virtual IPipelineResourceSignature* SHZ_CALL_TYPE GetPipelineResourceSignature() const override final
		{
			return GetSignature();
		}

		virtual bool SHZ_CALL_TYPE StaticResourcesInitialized() const override final
		{
		...
		virtual IShaderResourceVariable* SHZ_CALL_TYPE GetVariableByName(SHADER_TYPE ShaderType, const char* Name) override final
		{
			const PIPELINE_TYPE PipelineType = GetPipelineType();
		...
		virtual uint32 SHZ_CALL_TYPE GetVariableCount(SHADER_TYPE ShaderType) const override final
		{
			const PIPELINE_TYPE PipelineType = GetPipelineType();
			if (!IsConsistentShaderType(ShaderType, PipelineType))
			{
		...
		virtual IShaderResourceVariable* SHZ_CALL_TYPE GetVariableByIndex(SHADER_TYPE ShaderType, uint32 Index) override final
		{
			const PIPELINE_TYPE PipelineType = GetPipelineType();
			if (!IsConsistentShaderType(ShaderType, PipelineType))
			{
		...
		virtual void SHZ_CALL_TYPE BindResources(
		...
		virtual SHADER_RESOURCE_VARIABLE_TYPE_FLAGS SHZ_CALL_TYPE CheckResources(
			SHADER_TYPE ShaderStages,
			IResourceMapping* pResMapping,
	...
	private:
		...
		void ProcessVariables(SHADER_TYPE ShaderStages, HandlerType&& Handler) const
		{
			const PIPELINE_TYPE PipelineType = GetPipelineType();
