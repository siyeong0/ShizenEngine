...
#pragma once
 ...
 // \file
 // Implementation of the shz::PipelineResourceSignatureBase template class
...
#include "Engine/GraphicsUtils/Public/SRBMemoryAllocator.hpp"
...
namespace shz
{
	...
	void ValidatePipelineResourceSignatureDesc(
	...
	bool PipelineResourceSignaturesCompatible(
		const PipelineResourceSignatureDesc& Desc0,
	...
	void CopyPipelineResourceSignatureDesc(
	...
	public:
		...
		PipelineResourceSignatureDescWrapper(const char* PSOName, const PipelineResourceLayoutDesc& ResourceLayout, uint32 SRBAllocationGranularity)
		{
			if (PSOName != nullptr)
			{
	...
	public:
		...
		virtual IShaderResourceVariable* SHZ_CALL_TYPE GetStaticVariableByName(SHADER_TYPE ShaderType, const Char* Name) override final
		{
			if (!IsConsistentShaderType(ShaderType, m_PipelineType))
			{
		...
		virtual void SHZ_CALL_TYPE CreateShaderResourceBinding(IShaderResourceBinding** ppShaderResourceBinding, bool InitStaticResources) override final
		{
		...
		virtual void SHZ_CALL_TYPE CopyStaticResources(IPipelineResourceSignature* pDstSignature) const override final
		{
		...
		virtual bool SHZ_CALL_TYPE IsCompatibleWith(const IPipelineResourceSignature* pPRS) const override final
		{
			if (pPRS == nullptr)
				return IsEmpty();
		...
		{
			return shz::FindResource(this->m_Desc.Resources, this->m_Desc.NumResources, ShaderStage, ResourceName);
		}
		...
		{
			return shz::FindImmutableSampler(this->m_Desc.ImmutableSamplers, this->m_Desc.NumImmutableSamplers,
			...
			HandlerType&& Handler) const
		{
			...
			{
				const auto IdxRange = AllowedVarTypes != nullptr ? GetResourceIndexRange(AllowedVarTypes[TypeIdx]) : std::make_pair<uint32, uint32>(0, GetTotalResourceCount());
	...
	protected:
			...
			const AllocImmutableSamplerAttribsCallbackType& AllocImmutableSampler = nullptr) noexcept(false)
		{
			...
			m_pResourceAttribs = AllocResourceAttribs ? AllocResourceAttribs(Allocator) : Allocator.Allocate<PipelineResourceAttribsType>(Desc.NumResources);
			...
			m_pImmutableSamplerAttribs = AllocImmutableSampler ? static_cast<ImmutableSamplerAttribsType*>(AllocImmutableSampler(Allocator)) : Allocator.ConstructArray<ImmutableSamplerAttribsType>(Desc.NumImmutableSamplers);
			if (CreateImmutableSamplers)
			{
	...
	protected:
		...
		PipelineResourceSignatureDescWrapper DecoupleCombinedSamplers(const PipelineResourceSignatureDesc& Desc)
		{
			...
			for (uint32 r = 0; r < Desc.NumResources; ++r)
			{
				...
				if (shz::FindResource(Desc.Resources, Desc.NumResources, Res.ShaderStages, SamplerName.c_str()) == InvalidPipelineResourceIndex)
				{
