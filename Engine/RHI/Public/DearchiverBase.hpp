...
#pragma once
 ...
 // \file
 // Implementation of the shz::DearchiverBase class
...
namespace shz
{
	...
	public:
		...
		virtual bool SHZ_CALL_TYPE LoadArchive(const IDataBlob* pArchiveData, uint32 ContentVersion, bool MakeCopy) override final;

		// Implementation of IDearchiver::UnpackShader().
		virtual void SHZ_CALL_TYPE UnpackShader(const ShaderUnpackInfo& UnpackInfo, IShader** ppShader) override final;

		// Implementation of IDearchiver::UnpackPipelineState().
		virtual void SHZ_CALL_TYPE UnpackPipelineState(const PipelineStateUnpackInfo& DeArchiveInfo, IPipelineState** ppPSO) override final;

		// Implementation of IDearchiver::UnpackResourceSignature().
		virtual void SHZ_CALL_TYPE UnpackResourceSignature(const ResourceSignatureUnpackInfo& DeArchiveInfo, IPipelineResourceSignature** ppSignature) override final;

		// Implementation of IDearchiver::UnpackRenderPass().
		virtual void SHZ_CALL_TYPE UnpackRenderPass(const RenderPassUnpackInfo& DeArchiveInfo, IRenderPass** ppRP) override final;

		// Implementation of IDearchiver::Store().
		virtual bool SHZ_CALL_TYPE Store(IDataBlob** ppArchive) const override final;
	...
	RefCntAutoPtr<IPipelineResourceSignature> DearchiverBase::UnpackResourceSignatureImpl(
		const ResourceSignatureUnpackInfo& DeArchiveInfo, bool IsImplicit)
	{
