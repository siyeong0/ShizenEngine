...
#pragma once
 ...
 // \file
 // Implementation of the shz::PipelineStateBase template class
...
namespace shz
{
	...
	void ValidatePSOCreateInfo(const IRenderDevice* pDevice, const PSOCreateInfoType& CreateInfo) noexcept(false);

	// Validates graphics pipeline create attributes and throws an exception in case of an error.
	template <>
	void ValidatePSOCreateInfo<GraphicsPipelineStateCreateInfo>(const IRenderDevice* pDevice, const GraphicsPipelineStateCreateInfo& CreateInfo) noexcept(false);

	// Validates compute pipeline create attributes and throws an exception in case of an error.
	template <>
	void ValidatePSOCreateInfo<ComputePipelineStateCreateInfo>(const IRenderDevice* pDevice, const ComputePipelineStateCreateInfo& CreateInfo) noexcept(false);

	// Validates ray-tracing pipeline create attributes and throws an exception in case of an error.
	template <>
	void ValidatePSOCreateInfo<RayTracingPipelineStateCreateInfo>(const IRenderDevice* pDevice, const RayTracingPipelineStateCreateInfo& CreateInfo) noexcept(false);

	// Validates tile pipeline create attributes and throws an exception in case of an error.
	template <>
	void ValidatePSOCreateInfo<TilePipelineStateCreateInfo>(const IRenderDevice* pDevice, const TilePipelineStateCreateInfo& CreateInfo) noexcept(false);
	...
	void ValidatePipelineResourceCompatibility(
	...
	void CopyRTShaderGroupNames(
		std::unordered_map<HashMapStringKey, uint32>& NameToGroupIndex,
	...
	struct ShaderResourceHashKey : public HashMapStringKey
	{
		...
		ShaderResourceHashKey(const SHADER_TYPE _ShaderStages, ArgsType&&... Args) noexcept
	...
	namespace PipelineStateUtils
	{
		...
		template <typename ShaderImplType, typename TShaderStages>
		void ExtractShaders(
			const TilePipelineStateCreateInfo& CreateInfo,
			TShaderStages& ShaderStages,
			bool WaitUntilShadersReady,
			SHADER_TYPE& ActiveShaderStages)
		{
	...
	public:
		...
		~PipelineStateBase()
		{
			...
			//       are in dtor, the object is locked by shz::RefCountedObject::Release() and
		...
		virtual const PipelineStateDesc& SHZ_CALL_TYPE GetDesc() const override final
		{
			CheckPipelineReady();
			return this->m_Desc;
		}
		...
		virtual IShaderResourceVariable* SHZ_CALL_TYPE GetStaticVariableByName(SHADER_TYPE ShaderType,
		...
		virtual IShaderResourceVariable* SHZ_CALL_TYPE GetStaticVariableByIndex(SHADER_TYPE ShaderType, uint32 Index) override final
		{
			CheckPipelineReady();
		...
		virtual uint32 SHZ_CALL_TYPE GetStaticVariableCount(SHADER_TYPE ShaderType) const override final
		{
			CheckPipelineReady();
		...
		virtual void SHZ_CALL_TYPE BindStaticResources(
		...
		virtual void SHZ_CALL_TYPE CopyStaticResources(IPipelineState* pDstPipeline) const override final
		{
			CheckPipelineReady();
		...
		virtual uint32 SHZ_CALL_TYPE GetResourceSignatureCount() const override final
		{
			CheckPipelineReady();
	...
	protected:
		...
		void ReserveSpaceForPipelineDesc(const TilePipelineStateCreateInfo& CreateInfo, FixedLinearAllocator& MemPool) noexcept
		{
	...
	protected:
		...
		void InitializePipelineDesc(const TilePipelineStateCreateInfo& CreateInfo, FixedLinearAllocator& MemPool)
		{
		...
		struct ResourceAttribution
		{
			...
			ResourceAttribution(
				const PipelineResourceSignatureImplType* _pSignature,
		...
		static ResourceAttribution GetResourceAttribution(
			const char* Name,
			SHADER_TYPE Stage,
