...
#pragma once
 ...
 // \file
 // Implementation of the shz::TextureBase template class
...
namespace shz
{
	...
	public:
		...
		//
		// - Creates default shader resource view addressing the entire texture if shz::BIND_SHADER_RESOURCE flag is set.
		// - Creates default render target view addressing the most detailed mip level if shz::BIND_RENDER_TARGET flag is set.
		// - Creates default depth-stencil view addressing the most detailed mip level if shz::BIND_DEPTH_STENCIL flag is set.
		// - Creates default unordered access view addressing the entire texture if shz::BIND_UNORDERED_ACCESS flag is set.
		// - Creates default shading rate view addressing the most detailed mip if shz::BIND_SHADING_RATE flag is set.
		//
		...
		virtual RESOURCE_STATE SHZ_CALL_TYPE GetState() const override final
		{
			return this->m_State;
		}
		...
		virtual ITextureView* SHZ_CALL_TYPE GetDefaultView(TEXTURE_VIEW_TYPE ViewType) override
		{
