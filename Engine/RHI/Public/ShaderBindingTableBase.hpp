...
#pragma once
 ...
 // \file
 // Implementation of the shz::ShaderBindingTableBase template class

#include <unordered_map>
#include <cstring>

#include "Engine/RHI/Interface/IShaderBindingTable.h"
...
namespace shz
{
	...
	public:
		...
		void SHZ_CALL_TYPE Reset(IPipelineState* pPSO) override final
		{
#ifdef SHZ_DEBUG
		...
		void SHZ_CALL_TYPE ResetHitGroups() override final
		{
#ifdef SHZ_DEBUG
			this->m_DbgHitGroupBindings.clear();
#endif
		...
		void SHZ_CALL_TYPE BindHitGroupByIndex(
			uint32      BindingIndex,
		...
		void SHZ_CALL_TYPE BindHitGroupForGeometry(
			ITopLevelAS* pTLAS,
		...
		void SHZ_CALL_TYPE BindHitGroupForInstance(
			ITopLevelAS* pTLAS,
			const char* pInstanceName,
			uint32       RayOffsetInHitGroupIndex,
			const char* pShaderGroupName,
		...
		void SHZ_CALL_TYPE BindHitGroupForTLAS(
			ITopLevelAS* pTLAS,
			uint32       RayOffsetInHitGroupIndex,
			const char* pShaderGroupName,
			const void* pData,
		...
		void SHZ_CALL_TYPE BindCallableShader(
			const char* pShaderGroupName,
		...
		bool SHZ_CALL_TYPE Verify(VERIFY_SBT_FLAGS Flags) const override final
		{
#ifdef SHZ_DEBUG
