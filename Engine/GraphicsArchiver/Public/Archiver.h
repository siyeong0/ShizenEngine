...
#pragma once
 ...
 // \file
 // Defines shz::IArchiver interface
...
namespace shz
{
	...
	struct SHZ_INTERFACE IArchiver : public IObject
	{
		...
		virtual bool SerializeToBlob(uint32 ContentVersion, IDataBlob** ppBlob) = 0;
		...
		// \note
		//     The method is *not* thread-safe and **must not** be called from multiple threads simultaneously.
		virtual bool SerializeToStream(uint32       ContentVersion, IFileStream* pStream) = 0;
		...
		virtual bool AddShader(IShader* pShader) = 0;
		...
		//
		//     The method is thread-safe and may be called from multiple threads simultaneously.
		virtual bool AddPipelineState(IPipelineState* pPSO) = 0;
		...
		//
		//     The method is thread-safe and may be called from multiple threads simultaneously.
		virtual bool AddPipelineResourceSignature(IPipelineResourceSignature* pSignature) = 0;
		...
		virtual IShader* GetShader(const char* ShaderName) = 0;
		...
		// \note       The method does **NOT** increment the reference counter of the returned object,
		//             so the application **must not** call Release() unless it also explicitly calls AddRef().
		virtual IPipelineState* GetPipelineState(PIPELINE_TYPE PSOType, const char* PSOName) = 0;
		...
		// \note      The method does **NOT** increment the reference counter of the returned object,
		//             so the application **must not** call Release() unless it also explicitly calls AddRef().
		virtual IPipelineResourceSignature* GetPipelineResourceSignature(const char* PRSName) = 0;
	};
