...
#include "Engine/RHI/Public/DeviceObjectArchive.hpp"
...
namespace shz
{
	...
	public:
		...
		virtual bool SHZ_CALL_TYPE SerializeToBlob(uint32 ContentVersion, IDataBlob** ppBlob) override final;

		// Implementation of IArchiver::SerializeToStream().
		virtual bool SHZ_CALL_TYPE SerializeToStream(uint32 ContentVersion, IFileStream* pStream) override final;

		// Implementation of IArchiver::AddShader().
		virtual bool SHZ_CALL_TYPE AddShader(IShader* pShader) override final;

		// Implementation of IArchiver::AddPipelineState().
		virtual bool SHZ_CALL_TYPE AddPipelineState(IPipelineState* pPSO) override final;

		// Implementation of IArchiver::AddPipelineResourceSignature().
		virtual bool SHZ_CALL_TYPE AddPipelineResourceSignature(IPipelineResourceSignature* pSignature) override final;

		// Implementation of IArchiver::Reset().
		virtual void SHZ_CALL_TYPE Reset() override final;

		// Implementation of IArchiver::GetShader().
		virtual IShader* SHZ_CALL_TYPE GetShader(const char* Name) override final;

		// Implementation of IArchiver::GetPipelineState().
		virtual IPipelineState* SHZ_CALL_TYPE GetPipelineState(PIPELINE_TYPE PSOType,
			const char* PSOName) override final;

		// Implementation of IArchiver::GetPipelineResourceSignature().
		virtual IPipelineResourceSignature* SHZ_CALL_TYPE GetPipelineResourceSignature(const char* PRSName) override final;
