...
#include "Shaders/HLSL_Structures.hlsli"
	...
	void GrassViewer::Initialize(const SampleInitInfo& initInfo)
	{
		...
		// -----------------------------------------------------------------
		{
			...
			m_pVolumetricFogSystem = std::make_unique<VolumetricFogSystem>();

			m_pInteractionSystem->Initialize(*m_pRenderer);
			m_pGrassSystem->Initialize(*m_pRenderer);
			m_pVolumetricFogSystem->Initialize(*m_pRenderer);
			...
			// ------------------------------------------------------------
			{
				...
				auto addGrassVariation = [&](GrassDesc& gd, const std::string& path)
				{
					...
					// LOD1 : Cross-plane 
					{
						...
						{
							std::vector<float3> pos(8);
							std::vector<float2> uv(8);
							std::vector<float3> normal(8);
							std::vector<float3> tangent(8);
							...
							const Box& b = grassMesh[0].GetBoxBounds();
							const float h = (b.Max().y - b.Min().y);
							const float yScale = (1.0f / h);

							const float xExtent = (b.Max().x - b.Min().x);
							const float zExtent = (b.Max().z - b.Min().z);
							const float needWidth = std::max(xExtent, zExtent);
							...
							float x0 = -0.5f * quadSize;
							float x1 = +0.5f * quadSize;
							float y0 = -0.5f * quadSize + 0.5f;
							float y1 = +0.5f * quadSize + 0.5f;

							// ---------- Plane A (XY plane, facing +Z)
							...
							const float3 nA = float3{ 0.0f, 0.0f, 1.0f };
							const float3 tA = float3{ 1.0f, 0.0f, 0.0f };
							for (int i = 0; i < 4; ++i)
							{
								normal[i] = nA;
								tangent[i] = tA;
							...
							// ---------- Plane B : rotate +90deg around Y (cross)
							auto rotY90 = [](const float3& p) -> float3
							{
								// (x,z) -> (z,-x)
								return float3{ p.z, p.y, -p.x };
							};

							pos[4] = rotY90(pos[0]); uv[4] = uv[0];
							pos[5] = rotY90(pos[1]); uv[5] = uv[1];
							pos[6] = rotY90(pos[2]); uv[6] = uv[2];
							pos[7] = rotY90(pos[3]); uv[7] = uv[3];

							const float3 nB = float3{ 1.0f, 0.0f, 0.0f };
							const float3 tB = float3{ 0.0f, 0.0f, -1.0f };
							for (int i = 4; i < 8; ++i)
							{
								normal[i] = nB;
								tangent[i] = tB;
							...
							grassCrossMeshLevel.SetPositions(std::move(pos));
							grassCrossMeshLevel.SetTexCoords(std::move(uv));
							grassCrossMeshLevel.SetNormals(std::move(normal));
							grassCrossMeshLevel.SetTangents(std::move(tangent));
							grassCrossMeshLevel.SetIndicesU32(std::move(idx));
							...
							sec.BaseVertex = 0;
							sec.MaterialSlot = 0;
							grassCrossMeshLevel.SetSections(std::vector<StaticMeshLevel::Section>{ sec });

							grassCrossMeshLevel.RecomputeBounds();
						}

						MaterialId matId = MaterialManager::GetInstance()->CreateMaterial("GrassCrossPlane", "GrassCrossPlane");
						Material& mat = MaterialManager::GetInstance()->GetMaterial(matId);
						...
						const std::string baseColorPath = BuildImpostorAtlasPath(path, "_albedo_spritesheet.png");
						mat.SetTextureAssetRef("g_BaseColorTex", m_pAssetManager->RegisterAsset<Texture>(baseColorPath));
						const std::string normalPath = BuildImpostorAtlasPath(path, "_normal_spritesheet.png");
						...
						grassCrossMeshLevel.SetMaterialSlots(std::move(materials));
				...
				// Grass
				{
					...
					addGrassVariation(gd, "C:/Dev/ShizenEngine/Assets/Grass/wild/Grass_small_017.fbx");
	...
	void GrassViewer::Update(double currTime, double elapsedTime, bool doUpdateUI)
	{
		...
		m_pRenderer->UpdateBuffer<hlsl::FogConstants>(STRING_HASH("FogConstantsCB"), m_FogSettings);
	...
	void GrassViewer::UpdateUI()
	{
		...
		if (ImGui::Begin("Settings##GrassViewerSettings", nullptr, ImGuiWindowFlags_AlwaysAutoResize))
		{
			...
			// ---------------------------------------------------------------------
			// Volumetric Fog Settings UI
			// ---------------------------------------------------------------------
			ImGui::Separator();
			ImGui::Text("Volumetric Fog");

			static bool s_InitFogDefaults = false;

			auto ResetFogDefaults = [&]()
			{
				m_FogSettings.BaseDensity = 0.001f;
				m_FogSettings.DensityScale = 1.0f;
				m_FogSettings.ExtinctionScale = 0.85f;
				m_FogSettings.Albedo = 0.92f;

				m_FogSettings.AnisotropyG = 0.1f;
				m_FogSettings.PhaseBoost = 1.0f;

				m_FogSettings.FogColor = float3(0.78f, 0.86f, 1.00f);

				m_FogSettings.MaxDistance = 2500.0f;

				m_FogSettings.BaseHeight = 0.0f;
				m_FogSettings.HeightFogStart = 0.0f;
				m_FogSettings.HeightFalloff = 0.0f;
			...
			{
				s_InitFogDefaults = true;
				ResetFogDefaults();
			}

			// Reset button
			if (ImGui::Button("Reset Fog Defaults##GrassViewer"))
			{
				ResetFogDefaults();
			}

			// Grouped UI
			if (ImGui::CollapsingHeader("Density / Scattering##Fog", ImGuiTreeNodeFlags_DefaultOpen))
			{
				ImGui::DragFloat("BaseDensity (1/m)##Fog", &m_FogSettings.BaseDensity, 0.001f, 0.0f, 0.20f, "%.4f");
				ImGui::SliderFloat("DensityScale##Fog", &m_FogSettings.DensityScale, 0.0f, 5.0f, "%.3f");
				ImGui::SliderFloat("ExtinctionScale##Fog", &m_FogSettings.ExtinctionScale, 0.0f, 5.0f, "%.3f");

				ImGui::Separator();

				ImGui::SliderFloat("Albedo##Fog", &m_FogSettings.Albedo, 0.0f, 1.0f, "%.3f");
				ImGui::SliderFloat("Anisotropy G##Fog", &m_FogSettings.AnisotropyG, 0.0f, 0.95f, "%.3f");
				ImGui::SliderFloat("PhaseBoost##Fog", &m_FogSettings.PhaseBoost, 0.0f, 3.0f, "%.3f");

				float col[3] = { m_FogSettings.FogColor.x, m_FogSettings.FogColor.y, m_FogSettings.FogColor.z };
				if (ImGui::ColorEdit3("FogColor##Fog", col))
				{
					m_FogSettings.FogColor = float3{
						std::max(col[0], 0.0f),
						std::max(col[1], 0.0f),
						std::max(col[2], 0.0f)
					};
				...
				ImGui::TextDisabled("Tip: BaseDensity 0.01~0.03 (haze), 0.05+ (thick).");
				ImGui::TextDisabled("Tip: G 0.2~0.4 natural, 0.7+ strong god-rays.");
			}

			if (ImGui::CollapsingHeader("Range / Height Fog##Fog", ImGuiTreeNodeFlags_DefaultOpen))
			{
				ImGui::DragFloat("MaxDistance (m)##Fog", &m_FogSettings.MaxDistance, 5.0f, 0.0f, 5000.0f, "%.1f");

				ImGui::Separator();

				ImGui::DragFloat("BaseHeight (m)##Fog", &m_FogSettings.BaseHeight, 0.5f, -5000.0f, 5000.0f, "%.2f");
				ImGui::DragFloat("HeightFogStart (m)##Fog", &m_FogSettings.HeightFogStart, 0.5f, -5000.0f, 5000.0f, "%.2f");
				ImGui::DragFloat("HeightFalloff (1/m)##Fog", &m_FogSettings.HeightFalloff, 0.005f, 0.0f, 2.0f, "%.4f");

				ImGui::TextDisabled("HeightFalloff=0 disables height fog.");
				ImGui::TextDisabled("Suggested: HeightFalloff 0.05~0.25 for ground fog.");
			}

			if (ImGui::CollapsingHeader("Temporal / Jitter##Fog", ImGuiTreeNodeFlags_DefaultOpen))
			{
	...
	void GrassViewer::BuildSceneOnce()
	{
		...
		// -------------------------------------------------------------------------
		{
			const std::string path = "C:/Dev/ShizenEngine/Assets/Sponza/sponza.gltf";

			// 1) Load mesh once
			StaticMesh mesh = {};
			{
				AssetRef<AssimpAsset> ref = m_pAssetManager->RegisterAsset<AssimpAsset>(path);
			...
			flecs::entity e = ecs.entity();
			e.set<CName>({ "Sponza palace" });

			// Transform
			CTransform tr = {};
			tr.Position = { 845.0f, m_pTerrainSystem->SampleWorldHeight(845.0f, -2014.0f) + 1.0f, -2014.0f };
			tr.Rotation = { 0,0,0 };
			tr.Scale = { 1.0f, 1.0f, 1.0f };
			e.set<CTransform>(tr);
			...
			mr.bCastShadow = true;
			mr.RenderObjectHandle = m_pRenderScene->AddObject(
				m_pRenderer->CreateStaticMeshRenderData(mesh),
				Matrix4x4::TRS(tr.Position, tr.Rotation, tr.Scale),
				true);
			e.set<CMeshRenderer>(mr);
		}
