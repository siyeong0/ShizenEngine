...
namespace shz
{
	...
	static bool CreateDirectoryImpl(const shz::Char* strPath);

	bool WindowsFileSystem::CreateDirectory(const shz::Char* strPath)
	{
		return CreateDirectoryImpl(strPath);
	}

	static void DeleteFileImpl(const shz::Char* strPath);

	void WindowsFileSystem::DeleteFile(const shz::Char* strPath)
	{
...
#undef CALL_WIN_FUNC
	...
	{
		return shz::SearchRecursive<WindowsFileSystem>(Dir, SearchPattern);
	}
