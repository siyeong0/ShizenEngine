...
#include "Tutorial06_ShadowMap.h"
...
namespace shz
{
	...
	{
		return new Tutorial06_ShadowMap();
	}

	void Tutorial06_ShadowMap::CreateCubePSO()
	{
	...
	void Tutorial06_ShadowMap::ModifyEngineInitInfo(const ModifyEngineInitInfoAttribs& Attribs)
	{
	...
	void Tutorial06_ShadowMap::CreatePlanePSO()
	{
		GraphicsPipelineStateCreateInfo PSOCreateInfo;
	...
	void Tutorial06_ShadowMap::CreateShadowMapVisPSO()
	{
		GraphicsPipelineStateCreateInfo PSOCreateInfo;
	...
	void Tutorial06_ShadowMap::UpdateUI()
	{
	...
	void Tutorial06_ShadowMap::Initialize(const SampleInitInfo& InitInfo)
	{
		...
		// Load texture
		RefCntAutoPtr<ITexture> CubeTexture = TexturedCube::LoadTexture(m_pDevice, "Assets/pearl_abyss_logo.png");
	...
	void Tutorial06_ShadowMap::CreateShadowMap()
	{
		TextureDesc SMDesc;
	...
	void Tutorial06_ShadowMap::RenderShadowMap()
	{
		const Vector3 lightDir = Vector3::Normalize(m_LightDirection);

		// Choose an up vector that is NOT parallel to lightDir
		Vector3 up;
		{
			const float ax = std::abs(lightDir.x);
			const float ay = std::abs(lightDir.y);
			const float az = std::abs(lightDir.z);
		...
		// We place the eye at origin and look along light direction.
		const Vector3 eye = Vector3(0.0f, 0.0f, 0.0f);
		const Vector3 at = eye + lightDir;

		// Build light view matrix
		Matrix4x4 WorldToLightViewSpaceMatr = Matrix4x4::LookAtLH(eye, at, up);
	...
	void Tutorial06_ShadowMap::RenderCube(const float4x4& CameraViewProj, bool IsShadowPass)
	{
		// Update constant buffer
		{
			...
			float4x4 NormalMatrix = Matrix4x4::RemoveTranslation(m_CubeWorldMatrix).Inversed().Transposed();
			...
			CBConstants->LightDirection = { m_LightDirection.x, m_LightDirection.y, m_LightDirection.z, 0.0f };
		}
	...
	void Tutorial06_ShadowMap::RenderPlane()
	{
		{
			struct Constants
			{
	...
	void Tutorial06_ShadowMap::RenderShadowMapVis()
	{
	...
	// Render a frame
	void Tutorial06_ShadowMap::Render()
	{
		// Render shadow map
		...
		if (m_ConvertPSOutputToGamma)
		{
			...
			float3 ClearColor3 = LinearToSRGB(ClearColor);
			ClearColor = { ClearColor3.x, ClearColor3.y, ClearColor3.z, 0.0f };
		}
	...
	void Tutorial06_ShadowMap::Update(double CurrTime, double ElapsedTime, bool DoUpdateUI)
	{
		...
		float4x4 CameraView = float4x4::Translation({ 0.f, -5.0f, -10.0f }) * float4x4::RotationY(PI) * float4x4::RotationX(-PI * 0.2);
